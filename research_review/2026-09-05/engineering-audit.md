...
**Erratum — 6 September 2026:** The initial scanner diagnosis examined the internal summary rather than the complete CLI path. `dashboard/scan.py:270-281` already converts NaN/Infinity to null before writing JSON. The CLI does not have the claimed invalid-JSON defect; the finding below is corrected.
...
- Isolated scanner probe found NaN in an internal summary with a missing optional metric. Follow-up source verification established that the CLI sanitizes it before JSON output; this is not a scanner-CLI defect.
...
- **Scanner diagnosis corrected.** `dashboard/scan.py:103,113-114` uses NaN internally for absent optional metrics, but `main()` recursively sanitizes non-finite values to null before writing (:270-281). The isolated direct-serialization probe bypassed that protection and does not establish an invalid-JSON defect in the shipped CLI. Dashboard consumers should still handle null explicitly.
