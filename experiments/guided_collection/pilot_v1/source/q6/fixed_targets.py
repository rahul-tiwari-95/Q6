"""Paired fixed-data exact-Q versus Double-DQN target diagnostic.

Both arms see identical full-action batches. This is privileged offline coverage,
not an online RL or sample-efficiency benchmark. Snapshots are inference only.
"""
...
from .supervised import (BUCKETS, CHECKPOINTS, LOSS_FIELDS, STATE_FIELDS, TIE_ATOL,
                         BatchSampler, enumerate_panel, full_state_metrics, layout_key,
                         reduce_states, rollout, save_manifest, supervised_update)
...
def peak_rss_bytes():
    """ru_maxrss is bytes on Darwin and KiB on Linux; this is process RSS."""
...
class MemoryReached(BudgetReached):
    pass


def guard(deadline, max_rss_bytes):
    check_budget(deadline)
    if peak_rss_bytes() > max_rss_bytes:
        raise MemoryReached("process peak RSS exceeded the declared ceiling")


def select_fixed_layouts(config, train_seeds, fresh_count, fresh_start, previous_fresh, deadline=float("inf")):
    ...
    train_hashes = {r["layout_hash"] for r in train}
    previous_hashes = {r["layout_hash"] for r in previous_fresh}
    accepted = set()
    ...
    while len(fresh) < fresh_count:
        ...
        key = layout_key(env)
        reason = ("training_layout" if key in train_hashes else "previous_supervised_fresh_layout" if key in previous_hashes
                  else "earlier_fresh_layout" if key in accepted else None)
        ...
        if reason:
            ...
            accepted.add(key)
        candidate += 1
    counts = Counter(r["layout_hash"] for r in train)
    return {"train": train, "heldout": fresh, "collision_skips": skipped,
            "train_duplicate_layouts": {key: n for key, n in counts.items() if n > 1}}


def build_transitions(config, data, deadline=float("inf")):
    """Store actual kernel rewards/ends and compact same-bank successor indices.

    Dynamics are clock-independent except timeout: step the real kernel at one
    and two remaining ticks, then replicate the latter for all larger clocks.
    """
    n = len(data["observations"])
    successor = np.full((n, 4), -1, np.int32)
    rewards = np.empty((n, 4), np.float64)
    ends = np.empty((n, 4), bool)
    env = CollectionWorld(config)
    for seed in np.unique(data["map_seeds"]):
        check_budget(deadline)
        env.reset(seed=int(seed))
        rows = np.flatnonzero(data["map_seeds"] == seed)
        lookup = {(tuple(map(int, data["positions"][i])), int(data["remaining"][i])): int(i) for i in rows}
        for position in dict.fromkeys(tuple(map(int, data["positions"][i])) for i in rows):
            for action in range(4):
                results = {}
                for remaining in sorted({1, min(2, config.horizon)}):
                    env.position, env.elapsed = position, config.horizon - remaining
                    clone = env.clone()
                    _, reward, terminated, truncated, _ = clone.step(action)
                    results[remaining] = (clone.position, reward, terminated or truncated)
                for remaining in range(1, config.horizon + 1):
                    i = lookup[(position, remaining)]
                    nxt, reward, ended = results[min(remaining, 2)]
                    rewards[i, action], ends[i, action] = reward, ended
                    if not ended:
                        successor[i, action] = lookup[(nxt, remaining - 1)]
    return {"successor_indices": successor, "rewards": rewards, "ends": ends}


def validate_transitions(config, data, transitions, deadline=float("inf")):
    indices, ends, rewards = (transitions[k] for k in ("successor_indices", "ends", "rewards"))
    live = ~ends
    if not np.all(indices[ends] == -1) or np.any(indices[live] < 0) or np.any(indices[live] >= len(data["observations"])):
        raise ConsistencyError("invalid terminal/successor index")
    source = np.broadcast_to(np.arange(len(indices))[:, None], indices.shape)[live]
    destination = indices[live]
    if not np.array_equal(data["map_seeds"][source], data["map_seeds"][destination]) or not np.array_equal(
            data["remaining"][source] - 1, data["remaining"][destination]):
        raise ConsistencyError("successor crossed layout or failed clock decrement")
    bellman = rewards.copy()
    bellman[live] += config.gamma * data["targets"][destination].max(axis=1)
    residual = float(np.abs(bellman - data["targets"]).max())
    if residual > 1e-12:
        raise ConsistencyError("transition Bellman consistency failed")
    checked = 0
    env = CollectionWorld(config)
    for seed in np.unique(data["map_seeds"]):
        check_budget(deadline)
        env.reset(seed=int(seed))
        rows = np.flatnonzero(data["map_seeds"] == seed)
        for remaining in sorted({1, max(1, config.horizon // 2), config.horizon}):
            i = next(int(i) for i in rows if data["remaining"][i] == remaining)
            env.position = tuple(map(int, data["positions"][i]))
            env.elapsed = config.horizon - remaining
            for action in range(4):
                obs, reward, terminated, truncated, _ = env.clone().step(action)
                ended = terminated or truncated
                if ended != bool(ends[i, action]) or abs(reward - rewards[i, action]) > 1e-12:
                    raise ConsistencyError("saved transition differs from environment")
                if not ended and not np.array_equal(obs, data["observations"][indices[i, action]]):
                    raise ConsistencyError("successor observation differs from environment")
                checked += 1
    return {"action_targets_checked": int(indices.size), "maximum_bellman_residual": residual,
            "actual_kernel_crosschecks": checked, "terminal_indices_valid": True, "same_layout_clock_successors": True}


def double_dqn_targets(agent, observations, transitions, indices):
    """Online selects; lagged target evaluates. Terminal rows never index -1."""
    successors = transitions["successor_indices"][indices]
    live = ~transitions["ends"][indices]
    result = transitions["rewards"][indices].clone()
    with torch.no_grad():
        if live.any():
            following = observations[successors[live].long()]
            actions = agent.online(following).argmax(1, keepdim=True)
            values = agent.target(following).gather(1, actions).squeeze(1)
            result[live] += agent.gamma * values
    return result.detach()


def fixed_update(agent, observations, exact_targets, transitions, indices, condition):
    if condition == "exact_q":
        return supervised_update(agent, observations, exact_targets, indices)
    ...
    loss = F.smooth_l1_loss(agent.online(observations[indices]), targets, reduction="mean")
    agent.optimizer.zero_grad()
    loss.backward()
    torch.nn.utils.clip_grad_norm_(agent.online.parameters(), 5.0)
    agent.optimizer.step()
    with torch.no_grad():
        for lagged, online in zip(agent.target.parameters(), agent.online.parameters()):
            lagged.lerp_(online, 0.01)
...
def efficient_rate(rows, planner_steps):
    return sum(bool(r["success"]) and r["steps"] <= 2 * planner_steps[(r["panel"], r["map_seed"])] for r in rows) / len(rows)


def run_study(output, protocol_file, *, seeds=(0, 1, 2), updates=30000, train_maps=256, fresh_maps=64,
              train_seed_start=300000, fresh_seed_start=940000, max_seconds=1200, max_rss_bytes=4 * 1024**3,
              dashboard=None, prior_dir=None, smoke=False, checkpoints=None):
    started = time.monotonic()
    output, protocol_file = Path(output), Path(protocol_file)
    prior_dir = Path(prior_dir) if prior_dir is not None else ROOT / "experiments/supervised/pilot_v1"
    seeds = list(seeds)
    ...
    if min(updates, train_maps, fresh_maps, max_seconds, max_rss_bytes) <= 0:
    ...
    if prior_results["run"]["status"] != "complete":
        raise ValueError("a complete archived supervised study is required")
    config = WorldConfig()
    ...
    actual = dict(seeds=seeds, updates=updates, train_maps=train_maps, fresh_maps=fresh_maps,
                  ...
                  max_rss_bytes=max_rss_bytes, checkpoints=schedule)
    declared = dict(seeds=[0, 1, 2], updates=30000, train_maps=256, fresh_maps=64, train_seed_start=300000,
                    fresh_seed_start=940000, max_seconds=1200, max_rss_bytes=4 * 1024**3, checkpoints=list(CHECKPOINTS))
    ...
    protocol = {"id": "fixed-targets-v1", "question": "With fixed coverage, how do exact and bootstrapped targets compare?",
        "privileged_offline_coverage": True, "rule_visibility": "observed", "seeds": seeds, "checkpoints": schedule,
        "checkpoint_units": "optimizer updates", "conditions": [{"id": "exact_q", "label": "Exact targets"},
            {"id": "double_dqn", "label": "Bootstrapped Double DQN"}], "world": asdict(config), "runtime": runtime,
        ...
        "budget": {"maximum_updates": 2 * len(seeds) * updates, "updates_per_seed": updates, "batch_size": 64,
            "admission_seconds": max_seconds, "peak_process_rss_bytes": max_rss_bytes, "dataset_and_evaluation_included": True},
        ...
        "optimizer": {"name": "Adam", "learning_rate": .001, "loss": "all-four-action SmoothL1 mean", "beta": 1,
            ...
            "paired_batch_schedule": True, "double_dqn_gamma": .97, "target_tau": .01,
            "target_update_timing": "after each double_dqn optimizer step; exact_q target remains unused and unchanged"},
    ...
    command = ["python", "-m", "q6.fixed_targets", "--output", str(output), "--protocol-file", str(protocol_file),
    ...
    provenance = {"archive": artifact_path(prior_dir), "files": {name: sha(prior_dir / name) for name in
        ("dataset.npz", "dataset_metadata.json", "sampling.json", "results.json", "protocol.json", "manifest.json")},
        "archive_initial_policy_hashes": prior_results["run"]["initial_policy_hashes"], "exact_replication": [],
        "training_arrays_identical": None, "replication_required": not smoke and not deviations}
    ...
    dataset_seconds = reference_seconds = 0.
    ...
    with (output / "evaluations.csv").open("w", newline="") as ef, (output / "state_metrics.csv").open("w", newline="") as sf, \
        ...
        try:
            ...
            try:
                enforce()
                selection = select_fixed_layouts(config, protocol["dataset"]["train_map_seeds"], fresh_maps,
                                                 fresh_seed_start, prior_metadata["heldout"], deadline)
                ...
                metadata["split_check"] = {"layout_intersections": len({r["layout_hash"] for r in selection["train"]} &
                    ...
                    "previous_fresh_layout_intersections": len({r["layout_hash"] for r in prior_metadata["heldout"]} &
                    {r["layout_hash"] for r in selection["heldout"]})}
                del train_hashes, fresh_hashes
                if any(metadata["split_check"].values()):
                    raise ValueError("layout/observation leakage detected")
                ...
                if provenance["replication_required"] and not provenance["training_arrays_identical"]:
                    raise ConsistencyError("training bank differs from the archived supervised bank")
            ...
            for condition in CONDITIONS:
                for seed in seeds:
                    ...
                    sampler = samplers[active_key] = BatchSampler(len(train_obs), seed)
                    ...
                    for checkpoint in schedule:
                        ...
                        try:
                            while sampler.updates < checkpoint:
                                enforce()
                                loss = fixed_update(agent, train_obs, exact_targets, transition_tensors, sampler.next_batch(), condition)
                                total_updates += 1
                    ...
                    if condition == "exact_q":
                        archive_model = prior_dir / "models" / f"supervised_q_seed{seed}_update30000.pt"
                        ...
                        if provenance["replication_required"]:
                            ...
                            check.update(archive_model=artifact_path(archive_model), archive_model_sha256=sha(archive_model),
                                ...
                                batch_index_sha256_identical=sampler.digest.hexdigest() == prior_sampling[str(seed)]["batch_index_sha256"],
                                initial_weights_identical=initial_hashes[condition][str(seed)] == prior_results["run"]["initial_policy_hashes"][str(seed)])
                            if not all(check[k] for k in ("final_weights_identical", "batch_index_sha256_identical", "initial_weights_identical")):
                                provenance["exact_replication"].append(check)
                                raise ConsistencyError("exact arm failed archived deterministic replication")
                        provenance["exact_replication"].append(check)
            phase = time.monotonic()
            try:
    ...
    for (condition, seed), sampler in samplers.items():
        samples.setdefault(condition, {})[str(seed)] = {"updates": sampler.updates, "examples_seen": int(sampler.counts.sum()),
            "unique_states_sampled": int(np.count_nonzero(sampler.counts)), "batch_index_sha256": sampler.digest.hexdigest(), "rng_seed_tuple": [seed, 66301]}
    provenance["paired_consistency"] = []
    for seed in seeds:
        a, b = samplers.get(("exact_q", seed)), samplers.get(("double_dqn", seed))
        check = {"seed": seed, "complete": a is not None and b is not None and a.updates == b.updates == updates,
            "initial_weights_identical": initial_hashes.get("exact_q", {}).get(str(seed)) == initial_hashes.get("double_dqn", {}).get(str(seed))
                if a is not None and b is not None else None,
            "batch_index_sha256_identical": a.digest.hexdigest() == b.digest.hexdigest() if a is not None and b is not None else None,
            "per_row_counts_identical": bool(np.array_equal(a.counts, b.counts)) if a is not None and b is not None else None}
        provenance["paired_consistency"].append(check)
        if status == "complete" and not all(check[k] for k in ("complete", "initial_weights_identical", "batch_index_sha256_identical", "per_row_counts_identical")):
            status = "inconsistent_not_gate_evidence"
            provenance["stop_reason"] = "paired initialization or sampling consistency failed"
    ...
    paired = {"direction": "double_dqn minus exact_q", "scope": "final greedy, matched seed and layout", "per_seed": [], "per_layout": [], "aggregate": []}
    ...
    for seed in seeds:
        for panel in ("train", "heldout"):
            ...
            if set(pair) == set(CONDITIONS):
                paired["per_seed"].append({"seed": seed, "panel": panel, **{key + "_delta": pair["double_dqn"][key] - pair["exact_q"][key]
                    if pair["double_dqn"][key] is not None and pair["exact_q"][key] is not None else None
                    for key in ("success_rate", "mean_steps", "noop_rate", "efficient_success_rate")}})
            for layout in selection.get(panel, []):
                left = final_rows.get(("exact_q", seed, panel, layout["map_seed"]))
                right = final_rows.get(("double_dqn", seed, panel, layout["map_seed"]))
    ...
    interpretation = ("smoke_or_protocol_deviation_not_gate_evidence" if smoke or deviations else "incomplete_not_gate_evidence" if status != "complete"
        else "both_fresh_gates_met" if all(c["fresh"] for c in per_condition) else "exact_only_fresh_gate_met" if per_condition[0]["fresh"]
        else "bootstrap_only_fresh_gate_met" if per_condition[1]["fresh"] else "neither_fresh_gate_met")
    artifacts = {name: artifact_path(output / filename) for name, filename in {
        ...
        "sample_counts": "sample_counts.npz", "sampling": "sampling.json", "provenance": "provenance.json", "paired_differences": "paired_differences.json", "manifest": "manifest.json"}.items()}
    artifacts.update(directory=artifact_path(output), protocol_document=artifact_path(protocol_file), report="docs/experiments/fixed_targets_results_v1.md")
    result = {"schema_version": 1, "protocol": protocol, "run": {"id": output.name, "status": status, "interpretation": interpretation,
        ...
        "wall_seconds": time.monotonic() - started, "dataset_wall_seconds": dataset_seconds, "reference_wall_seconds": reference_seconds,
        ...
        "peak_rss_bytes": peak_rss_bytes(), "resource_checks": resource_checks, "resource_limits": {"peak_process_rss_bytes": max_rss_bytes, "seconds": max_seconds, "torch_threads": 1,
            "check_cadence": "each optimizer update, each evaluation layout, phase boundaries and after aggregation; sampled stop guard"},
        ...
        "limitations": ["Both arms have privileged enumerated full-action offline coverage; this is not an online RL benchmark.",
            "The comparison isolates these two target procedures under this data and optimizer, not all possible bootstrapping methods.",
            "Three initializations share one training bank; seed ranges are not confidence intervals.",
            "Low bootstrapped training loss can coexist with large exact-Q error."]},
        ...
        "sampling": samples, "provenance": provenance, "paired_differences": paired, "trajectories": trajectories, "artifacts": artifacts}
...
def main():
    ...
    parser.add_argument("--fresh-seed-start", type=int, default=940000)
    ...
    if args.smoke:
        ...
        args.train_seed_start, args.fresh_seed_start = 710000, 980000
    result = run_study(args.output, args.protocol_file, seeds=[int(v) for v in args.seeds.split(",")], updates=args.updates,
        ...
        max_seconds=args.max_seconds, max_rss_bytes=args.max_rss_bytes, dashboard=args.dashboard, prior_dir=args.prior_dir, smoke=args.smoke,
