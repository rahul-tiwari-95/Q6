"""Run a locally predeclared, exploratory A→B→A baseline pilot."""
...
def run_study(output: Path, seeds: list[int], phase_steps: int = 12000,
              ...
              dashboard: Path | None = None, eval_seed_start: int = 900000,
    ...
    if eval_seed_start < 900000:
        raise ValueError("evaluation seeds must be >=900000, disjoint from training seeds")
    ...
    config = WorldConfig()
    panel = list(range(eval_seed_start, eval_seed_start + eval_episodes))
    protocol_path = protocol_file or ROOT / "docs/experiments/adaptation_protocol_v1.md"
    if protocol_file is not None and not protocol_file.is_file():
        raise ValueError(f"protocol file does not exist: {protocol_file}")
    ...
    protocol_id = protocol_path.stem.replace("adaptation_protocol_", "adaptation-")
    protocol = {"id": protocol_id, "status": "exploratory_pilot", "registration": "locally_predeclared_not_externally_registered",
    ...
    command = ["python3", "-m", "q6.adaptation", "--output", str(output), "--seeds",
               ...
               "--eval-seed-start", str(eval_seed_start), "--protocol-file", str(protocol_path)]
    ...
    with (output / "training.csv").open("w", newline="") as training_file:
        ...
        for seed in seeds:
            ...
            parameter_count = sum(p.numel() for p in agent.online.parameters())
    ...
    try:
        protocol_document = protocol_path.resolve().relative_to(ROOT).as_posix()
    except ValueError:
    ...
    result = {"schema_version": 1, "protocol": protocol,
              "run": {"id": output.name, "status": status, "exploratory": True,
                      ...
                      "competence_note": "The locally predeclared feasibility gate is mean A success >=70% after A. Below it, retention is not interpreted as forgetting.",
...
def main():
    ...
    result = run_study(args.output, [int(s) for s in args.seeds.split(",")],
                       ...
                       args.eval_seed_start, args.protocol_file)
