...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("experiments/provenance/learning-lifecycle-corrected.md"))
    ...
    if forward_storm_bins:
        ...
        if prefers_no_mitigate_in_storm:
            lines.append("\n**The both-features learner correctly learned NOT to mitigate in every "
                         "forward-storm bin it visited** -- it used the supplied unique-count feature "
                         "to learn an action preference from reward feedback, with nothing hand-coding which feature "
                         "to trust.")
        else:
    ...
    lines.append("\nThis learner receives an already-deduplicated feature; it does not discover lineage from raw messages. These are frozen evaluations from one training-seed sequence, not evidence of online adaptation or convergence. Missing wealth, stocks, memory and the short discount horizon are possible contributors to the performance gap; no ablation here isolates their effects. The learning-rate floor becomes constant after 20 visits. Final transition rewards are now consumed by the episodic lifecycle; this is a new run, not a reproduction of the original historical learner report.")
