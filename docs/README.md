...
- [Guided-collection protocol](experiments/guided_collection_protocol_v1.md), [result](experiments/guided_collection_results_v1.md) and [validation](validation/guided-collection-v1.md): replacing half of random collection with frozen greedy episodes improves logged efficient-route availability 60.94% → 82.29%, but lowers fresh efficiency 49.13% → 43.32%. Keep the original random DDQN baseline; next isolate guidance from lost exploration using the existing eight-random-only logs.

- [Familiar-start protocol](experiments/familiar_starts_protocol_v1.md), [result](experiments/familiar_starts_results_v1.md) and [validation](validation/familiar-starts-v1.md): freezing both learners and restricting choices to logged actions cuts DDQN efficiency 68.49% → 40.76%, versus exact-label 25.82% → 24.05%. The regression survives familiar maps and masks; the logged efficient-route ceiling is 60.94%. The subsequent guided-collection comparison tests more goal-directed collection.

- [Exact logged-graph protocol](experiments/logged_graph_protocol_v1.md), [result](experiments/logged_graph_results_v1.md) and [validation](validation/logged-graph-v1.md): exact recorded-graph targets improve value MAE 0.2473 → 0.1134 but reduce fresh efficient success 48.70% → 18.64%, with declines in all 24 bank-panel means. The subsequent familiar-start diagnostic compares unrestricted versus logged-mask actions without training.
