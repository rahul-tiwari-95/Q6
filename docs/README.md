...
- [Constrained-bootstrap protocol](experiments/constrained_bootstrap_protocol_v1.md): restrict training-time successor action selection to logged actions, with recorded data, state replay, loss and target counts fixed; fresh evaluation remains unrestricted.
- [Recorded-action protocol](experiments/recorded_actions_protocol_v1.md), [result](experiments/recorded_actions_results_v1.md) and [validation](validation/recorded-actions-v1.md): exact state replay with logged-only action outcomes reduces efficient success 43.03% → 21.29%, with declines in all three banks; next is a constrained successor-action bootstrap control using the same recorded data.
