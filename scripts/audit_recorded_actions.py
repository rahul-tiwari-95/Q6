"""Independent audit of learning from deduplicated recorded actions.
...
forward regeneration for those saved recordings.
"""
...
def reconstruct_panels(study, archives, protocol, result, metadata, World, config):
    ...
    for name, directory in archives.items():
        if name in ("panel_evaluation", "bank_replication", "map_replay", "within_map"):
...
def validate_models(study, archive, result):
    ...
    expected_snapshots = {(b, "recorded_actions", s, cp) for b in banks for s in seeds for cp in schedule}
...
def validate_losses(study, result):
    ...
    for bank in protocol["bank_ids"]:
        for seed in protocol["seeds"]:
            ...
            previous = 0
            for row in group:
                assert row["condition"] == "recorded_actions"
    ...
    for row in result["loss_aggregate"]:
        ...
        assert row["condition"] == "recorded_actions" and row["seeds"] == len(group) == len(protocol["seeds"])
...
def validate_provenance(study, archives, result, data, transitions, supports, initial, snapshots, counts, rows, refs):
    ...
    for name, directory in archives.items():
        ...
        metadata_name = "panels.json" if name in ("panel_evaluation", "bank_replication", "map_replay", "within_map") else "dataset_metadata.json"
    ...
    for bank in banks:
        ...
        for condition in CONDITIONS:
            ...
            old_coverage = next(r for r in old["coverage"]["per_condition"] if r["condition"] == CONDITIONS[0])
...
def validate_exposure(study, result, data, transitions, supports, counts, recorded):
    ...
    for summary in exposure["summaries"]:
        ...
        structural = summary["all_action_structural_successor_queries"] if condition == CONDITIONS[1] else summary["successor_queries"]
...
def validate_action_exposure(study, result, data, transitions, supports, counts, tables):
    ...
    assert set(saved_queries) == {f"bank{b}_{CONDITIONS[1]}_seed{s}" for b, s in fits}
    ...
    for row in exposure["per_seed"]:
        ...
        if condition == CONDITIONS[1]:
            ...
            observed, ended, successor = np.ones_like(transitions["ends"]), transitions["ends"], transitions["successor_indices"]
        live = observed & ~ended
        ...
        if condition == CONDITIONS[1]:
            ...
            actual = saved_queries[f"bank{bank}_{CONDITIONS[1]}_seed{seed}"]
...
def main():
    ...
    parser.add_argument("--study", type=Path, default=Path("experiments/recorded_actions/pilot_v1"))
    parser.add_argument("--allow-smoke", action="store_true")
    parser.add_argument("--skip-forward-inference", action="store_true")
    archive_names = ("supervised", "fixed_targets", "coverage", "equal_support", "panel_evaluation", "bank_replication", "map_replay", "within_map")
    ...
    assert protocol["id"] == "recorded-actions-v1" and result["run"]["status"] == "complete"
    assert result["artifacts"]["report"] == "docs/experiments/recorded_actions_results_v1.md"
    assert protocol["bank_ids"] == [1, 2, 3]
    assert [r["id"] for r in protocol["conditions"]] == ["collected_unique", "recorded_actions"]
    assert protocol["primary_comparison"] == "recorded_minus_collected"
    final = max(protocol["checkpoints"])
    assert protocol["evaluation_checkpoints"] == {"collected_unique": 30000, "recorded_actions": final}
    if not smoke:
        ...
        assert protocol["panel_selection"] == {"count": 8, "maps_per_panel": 64, "start": 1080000, "stride": 1000}
    ...
    assert protocol["recorded_actions"]["equal_action_target_budget"] is False
    ...
    assert run["interpretation"] == ("smoke_or_deviation_descriptive_only" if smoke else "recorded_actions_descriptive_only") and run["stop_reason"] is None
    ...
    assert len(progress) == len(fits) * len(protocol["checkpoints"]) and all(r["condition"] == "recorded_actions" for r in progress)
    evaluated = [r for r in run["progress"] if r.get("evaluation_complete")]
    expected_evaluated = {(b, c, s, p["id"]) for b, s in fits for c in ("collected_unique", "recorded_actions") for p in result["panels"]}
