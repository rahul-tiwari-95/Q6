...
// Run from any directory: node /path/to/Q6/scripts/check_dashboard.mjs [competence-results.json] [--supervised supervised-results.json] [--missing-supervised] [--fixed fixed-targets-results.json] [--missing-fixed] [--coverage coverage-results.json] [--missing-coverage] [--equal-support results.json] [--missing-equal-support] [--panels results.json] [--missing-panels] [--banks results.json] [--missing-banks] [--map-replay results.json] [--missing-map-replay] [--within-map results.json] [--missing-within-map] [--recorded-actions results.json] [--missing-recorded-actions] [--constrained results.json] [--missing-constrained] [--logged-graph results.json] [--missing-logged-graph] [--familiar-starts results.json] [--missing-familiar-starts] [--guided-collection results.json] [--missing-guided-collection]
...
for(let i=2;i<process.argv.length;i++){
 ...
 else if(arg==='--map-replay'){mapReplayPath=process.argv[++i];assert(mapReplayPath,'--map-replay needs a real results path');}
 ...
 else if(arg==='--constrained'){constrainedPath=process.argv[++i];assert(constrainedPath,'--constrained needs a real results path');}
 else if(arg==='--missing-constrained')missingConstrained=true;
 else if(arg==='--logged-graph'){loggedGraphPath=process.argv[++i];assert(loggedGraphPath,'--logged-graph needs a real results path');}
 else if(arg==='--missing-logged-graph')missingLoggedGraph=true;
 else if(arg==='--familiar-starts'){familiarPath=process.argv[++i];assert(familiarPath,'--familiar-starts needs a real results path');}
 else if(arg==='--missing-familiar-starts')missingFamiliar=true;
 else if(arg==='--guided-collection'){guidedPath=process.argv[++i];assert(guidedPath,'--guided-collection needs a real results path');}
 else if(arg==='--missing-guided-collection')missingGuided=true;
...
class Element {
  constructor(id='',tag='div',attrs='') {this.id=id;this.tag=tag;this.listeners={};this.listenerCounts={};this.attributes={};this.dataset={};this._value='';this.options=[];this.hidden=/\bhidden\b/.test(attrs);this.disabled=false;this.width=560;this.classList={toggle(){}};for(const [,k,v] of attrs.matchAll(/([\w-]+)="([^"]*)"/g)){this.attributes[k]=v;if(k.startsWith('data-'))this.dataset[k.slice(5)]=v;}this._html='';}
  addEventListener(name,cb){this.listenerCounts[name]=(this.listenerCounts[name]||0)+1;this.listeners[name]=cb;}
...
const sandbox={document,location:{hash:''},history:{replaceState(){}},setInterval:callback=>{const id=++intervalId;intervals.set(id,callback);return id;},clearInterval(id){intervals.delete(id);},console,fetch:async relative=>{
 ...
 if(relative==='data/constrained_bootstrap.json'&&missingConstrained)return {status:404,ok:false};
 if(relative==='data/logged_graph.json'&&missingLoggedGraph)return {status:404,ok:false};
 if(relative==='data/familiar_starts.json'&&missingFamiliar)return {status:404,ok:false};
 if(relative==='data/guided_collection.json'&&missingGuided)return {status:404,ok:false};
 const override=relative==='data/competence.json'?competencePath:relative==='data/supervised.json'?supervisedPath:relative==='data/fixed_targets.json'?fixedPath:relative==='data/coverage.json'?coveragePath:relative==='data/equal_support.json'?equalSupportPath:relative==='data/panel_evaluation.json'?panelsPath:relative==='data/bank_replication.json'?banksPath:relative==='data/map_replay.json'?mapReplayPath:relative==='data/within_map.json'?withinMapPath:relative==='data/recorded_actions.json'?recordedPath:relative==='data/constrained_bootstrap.json'?constrainedPath:relative==='data/logged_graph.json'?loggedGraphPath:relative==='data/familiar_starts.json'?familiarPath:relative==='data/guided_collection.json'?guidedPath:null;
...
if(missingConstrained || (!constrainedPath&&!fs.existsSync(path.join(root,'dashboard/data/constrained_bootstrap.json'))))expectedMissing.push('data/constrained_bootstrap.json');
if(missingLoggedGraph || (!loggedGraphPath&&!fs.existsSync(path.join(root,'dashboard/data/logged_graph.json'))))expectedMissing.push('data/logged_graph.json');
if(missingFamiliar || (!familiarPath&&!fs.existsSync(path.join(root,'dashboard/data/familiar_starts.json'))))expectedMissing.push('data/familiar_starts.json');
if(missingGuided || (!guidedPath&&!fs.existsSync(path.join(root,'dashboard/data/guided_collection.json'))))expectedMissing.push('data/guided_collection.json');
...
assert.equal(get('coverage-study').value,'guided_collection','Guided collection is the default study');
assert.equal(get('banks-view').hidden,false);assert.equal(get('familiar-view').hidden,true);get('coverage-study').value='familiar_starts';get('coverage-study').listeners.change();
if(debug.familiar?.aggregate?.length){
 const data=debug.familiar,bankIds=[...new Set(data.aggregate.map(r=>r.bank_id))],conditions=['constrained_bootstrap','logged_graph'],actionSets=['unrestricted','logged'];
 assert.equal(get('familiar-view').hidden,false);assert.equal(get('banks-view').hidden,true);assert.equal(get('coverage-content').hidden,true);assert(get('familiar-note').textContent.includes(data.protocol.smoke?'Smoke':'Privileged familiar-start'));
 assert(html.includes('not fresh-map generalization'));
 if(data.robustness.eligible){for(const [key,label] of [['positive_banks','positive banks'],['negative_banks','negative'],['zero_banks','tied']])assert(get('familiar-effects-caption').textContent.includes(`${data.robustness[key]} ${label}`));}else assert(get('familiar-effects-caption').textContent.includes('not evidence'));
 const expected=bankIds.map(bank=>data.paired_differences.aggregate.find(r=>r.bank_id===bank&&r.block==='all'&&r.comparison==='interaction')?.efficient_success_delta);expected.push(data.pooled.paired.find(r=>r.block==='all'&&r.comparison==='interaction')?.efficient_success_delta);
 const pp=v=>Number.isFinite(v)?`${v>0?'+':''}${(100*v).toFixed(1)} pp`:'—';for(const rows of [data.paired_differences.aggregate,data.pooled.paired])for(const r of rows.filter(r=>r.block==='all'&&['mask_effect_constrained','mask_effect_graph','graph_minus_constrained_logged'].includes(r.comparison)))assert(get('familiar-effects').innerHTML.includes(pp(r.efficient_success_delta)));assert(html.includes('or hurts it less'));assert(!html.includes('Positive values mean greater rescue'));assert.deepEqual([...get('familiar-effects').innerHTML.matchAll(/<strong[^>]*>([^<]*)<\/strong>/g)].map(m=>m[1]),expected.map(pp));
 for(const bank of bankIds){
  get('familiar-bank').value=String(bank);get('familiar-bank').listeners.change();const rows=data.aggregate.filter(r=>r.bank_id===bank&&r.block==='all');assert.equal(rows.length,4);
  for(const r of rows){assert(get('familiar-matrix').innerHTML.includes(`${(100*r.success_rate).toFixed(1)}%`));assert(get('familiar-matrix').innerHTML.includes(`${(100*r.efficient_success_rate).toFixed(1)}%`));assert(get('familiar-matrix').innerHTML.includes(r.mean_steps.toFixed(3)));for(const k of ['off_mask_actions','supported_steps','unsupported_steps','evaluation_steps','support_exit_episodes'])assert(get('familiar-support-table').innerHTML.includes(`<td>${r[k].toLocaleString()}</td>`));if(r.action_set==='logged'){assert.equal(r.off_mask_actions,0);assert.equal(r.unsupported_steps,0);assert.equal(r.support_exits,0);}}
  const rate=(c,a)=>rows.find(r=>r.condition===c&&r.action_set===a).efficient_success_rate,interaction=rate(conditions[1],actionSets[1])-rate(conditions[1],actionSets[0])-rate(conditions[0],actionSets[1])+rate(conditions[0],actionSets[0]);assert(Math.abs(interaction-expected[bankIds.indexOf(bank)])<1e-12,'Saved interaction equals the four matched cells');
  const blockRows=data.aggregate.filter(r=>r.bank_id===bank&&r.block!=='all');for(const id of ['familiar-success-chart','familiar-efficient-chart']){assert.equal((get(id).innerHTML.match(/<circle /g)||[]).length,blockRows.length);assert(get(id).innerHTML.includes('Original training-map blocks'));assert(!get(id).innerHTML.includes('Fixed evaluation panels'));}
  assert.equal((get('familiar-learner-table').innerHTML.match(/<tr>/g)||[]).length,data.seed_results.filter(r=>r.bank_id===bank&&r.block==='all').length+1);
  const reach=data.reachability.per_bank.find(r=>r.bank_id===bank);for(const key of ['success_reachable_starts','efficient_success_reachable_starts'])assert(get('familiar-reference-note').textContent.includes(`${reach[key].toLocaleString()} / ${reach.starts.toLocaleString()}`));
  for(const axis of [...get('familiar-slice-axis').options]){get('familiar-slice-axis').value=axis.value;get('familiar-slice-axis').listeners.change();const slices=data.prediction_slices.per_slice.filter(r=>r.bank_id===bank&&r.axis===axis.value);assert.equal((get('familiar-slices-table').innerHTML.match(/<tr>/g)||[]).length,slices.length+1);for(const r of slices){assert(get('familiar-slices-table').innerHTML.includes(`<td>${r.states.toLocaleString()}</td>`));for(const key of ['state_mean_signed_error','centered_state_mean_squared_error','mean_target_top_two_gap'])if(Number.isFinite(r[key]))assert(get('familiar-slices-table').innerHTML.includes(r[key].toPrecision(4)));}if(axis.value==='original_start')assert(get('familiar-slices-note').textContent.includes('exploratory'));}
  get('familiar-slice-axis').value='action_count';get('familiar-slice-axis').listeners.change();
 }
 let recordings=0;const matched=new Set();
 for(const bank of bankIds){get('familiar-bank').value=String(bank);get('familiar-bank').listeners.change();for(const map of [...get('familiar-replay-map').options]){get('familiar-replay-map').value=map.value;get('familiar-replay-map').listeners.change();for(const controller of [...get('familiar-replay-controller').options]){get('familiar-replay-controller').value=controller.value;get('familiar-replay-controller').listeners.change();for(const actionSet of [...get('familiar-replay-action-set').options]){get('familiar-replay-action-set').value=actionSet.value;get('familiar-replay-action-set').listeners.change();for(const seed of [...get('familiar-replay-seed').options]){get('familiar-replay-seed').value=seed.value;get('familiar-replay-seed').listeners.change();const t=debug.familiarSelected;assert(t);matched.add(t);assert.equal(t.map_seed,Number(map.value));assert.equal(t.mode,'greedy');assert(get('familiar-world-caption').textContent.includes('Rule A'));assert(get('familiar-mask-diagnostics').innerHTML.includes('Pre-action recorded support'));if(t.policy==='learner'){assert(get('familiar-step-diagnostics').innerHTML.includes('Learned Q'));assert(get('familiar-mask-diagnostics').innerHTML.includes('Exact logged-graph Q'));}debug.familiarFrame=t.steps.length;debug.drawFamiliarWorld();assert(get('familiar-replay-step').textContent.includes(String(t.steps.length)));recordings++;}}}}}
 assert.equal(matched.size,data.trajectories.length,'Every exported familiar recording is reachable, including bank graph and shared planner references');
 get('familiar-replay-reset').listeners.click();get('familiar-replay-play').listeners.click();assert.equal(intervals.size,1);[...intervals.values()][0]();get('familiar-replay-play').listeners.click();assert.equal(intervals.size,0);get('familiar-replay-scrub').listeners.input({target:{value:'0'}});get('familiar-replay-play').listeners.click();debug.activateTab('adaptation');assert.equal(intervals.size,0);debug.activateTab('coverage');
 const savedRun=structuredClone(data.run),savedProtocol=structuredClone(data.protocol);data.run.status='incomplete';debug.renderFamiliar();assert(get('familiar-note').textContent.includes('Incomplete'));data.run.status='inconsistent_not_gate_evidence';data.run.stop_reason='<unsafe>reason';debug.renderFamiliar();assert(get('familiar-note').textContent.includes('Consistency check failed'));assert(get('familiar-note').textContent.includes('<unsafe>reason'));data.run.status='complete';data.protocol.smoke=true;debug.renderFamiliar();assert(get('familiar-note').textContent.includes('Smoke'));data.protocol.smoke=false;data.protocol.deviations=['changed budget'];debug.renderFamiliar();assert(get('familiar-note').textContent.includes('protocol-deviation'));data.run=savedRun;data.protocol=savedProtocol;
 const savedAggregate=data.aggregate;data.aggregate=[];debug.renderFamiliar();assert.equal(get('familiar-content').hidden,true);assert.equal(get('familiar-empty').hidden,false);data.aggregate=savedAggregate;debug.renderFamiliar();
 const firstMap=data.protocol.replay_map_seeds?.[0] ?? Math.min(...data.trajectories.map(r=>r.map_seed));get('coverage-study').listeners.change();assert.equal(debug.familiarSelected.map_seed,firstMap);get('familiar-replay-play').listeners.click();await debug.loadData();assert.equal(intervals.size,0);assert.equal(get('coverage-study').value,'familiar_starts');
 console.log(`Familiar starts: all ${matched.size} distinct recordings (${recordings} selector paths), four cells per bank, paired interactions, familiar-block charts, support/mask diagnostics, status branches, playback and refresh passed.`);
}else{assert.equal(get('familiar-view').hidden,false);assert.equal(get('familiar-content').hidden,true);assert.equal(get('familiar-empty').hidden,false);console.log('Missing familiar-start data stays empty; no earlier result substituted.');}

for(const bankStudy of ['guided_collection','logged_graph','constrained_bootstrap','recorded_actions','within_map','map_replay','bank_replication']){
get('coverage-study').value=bankStudy;get('coverage-study').listeners.change();
if(debug.banks?.aggregate?.length){
 const data=debug.banks,records=data.banks,conditions=bankStudy==='guided_collection'?['constrained_bootstrap','guided_collection']:bankStudy==='logged_graph'?['constrained_bootstrap','logged_graph']:bankStudy==='constrained_bootstrap'?['recorded_actions','constrained_bootstrap']:['collected_unique',bankStudy==='recorded_actions'?'recorded_actions':bankStudy==='within_map'?'within_map_uniform':bankStudy==='map_replay'?'map_balanced':'uniform_subset'],panels=data.protocol.panels.map(p=>p.id),signedBank=value=>Number.isFinite(value)?`${value>0?'+':''}${(100*value).toFixed(1)} pp`:'—';
 ...
 const checkBank=bank=>{
  ...
  if(bankStudy==='guided_collection'){for(const c of coverage)assert(get('banks-composition-caption').textContent.includes(c.unique_current_states.toLocaleString()));assert(get('banks-composition-caption').textContent.includes('sizes need not match'));}else{assert(get('banks-composition-caption').textContent.includes(bank.support_size.toLocaleString()));assert(get('banks-composition-caption').textContent.includes(collection.collection_episodes.toLocaleString()));assert(get('banks-composition-caption').textContent.includes(collection.collection_steps.toLocaleString()));}
  ...
  if(bankStudy==='guided_collection'){
   assert.equal(get('guided-collection-panel').hidden,false);assert.equal(get('guided-collection-replay-panel').hidden,false);
   const costs=data.collection.per_condition.filter(r=>r.bank_id===bank.bank_id);assert.equal(costs.length,2);
   for(const r of costs){for(const key of ['complete_collection_episodes','collection_steps'])assert(get('guided-collection-costs').innerHTML.includes(r[key].toLocaleString()));for(const component of r.per_policy)for(const key of ['episodes','complete_episodes','steps','successes','noop_steps','unique_trajectories'])assert(get('guided-collection-policy-table').innerHTML.includes(`<td>${component[key].toLocaleString()}</td>`));}
   assert.equal(costs[0].complete_collection_episodes,costs[1].complete_collection_episodes,'Complete episode allocation matches; actual steps are allowed to differ');
   for(const key of ['historical_interactions','historical_updates','historical_state_presentations','historical_action_targets'])assert(get('guided-collector-provenance').textContent.includes(data.collector[key].toLocaleString()));assert(get('guided-collector-provenance').textContent.includes('verified unchanged'));assert(get('guided-collector-provenance').textContent.includes('history includes the random slots replaced'));
   const routes=data.route_ceilings.per_condition.filter(r=>r.bank_id===bank.bank_id);assert.equal((get('guided-route-ceilings').innerHTML.match(/class="diagnostic-row"/g)||[]).length,4);for(const r of routes)for(const key of ['success_reachable_starts','efficient_success_reachable_starts'])assert(get('guided-route-ceilings').innerHTML.includes(`${r[key].toLocaleString()} / ${r.starts.toLocaleString()} original starts`));assert(get('guided-route-ceilings-note').textContent.includes('separate evaluation population'));
   assert.equal((get('guided-action-bars').innerHTML.match(/class="diagnostic-row"/g)||[]).length,8);for(const r of bank.recorded_actions.per_condition){assert(get('guided-action-caption').textContent.includes(r.supported_states.toLocaleString()));assert(get('guided-action-caption').textContent.includes(r.recorded_edges.toLocaleString()));}
   for(const r of data.action_exposure.per_seed.filter(r=>r.bank_id===bank.bank_id))for(const key of ['updates','state_presentations','action_target_presentations','terminal_action_targets','nonterminal_target_queries','outside_support_target_queries'])assert(get('guided-targets-table').innerHTML.includes(`<td>${r[key].toLocaleString()}</td>`));
   const check=data.provenance.random_slot_replication.find(r=>r.bank_id===bank.bank_id);assert(get('guided-collection-integrity').textContent.includes(check.compared_steps.toLocaleString()));assert(get('guided-collection-integrity').textContent.includes('verified identical'));check.identical=false;debug.renderGuidedCollection();assert(get('guided-collection-integrity').textContent.includes('verification failed'));check.identical=true;const savedCollector=data.collector.unchanged;data.collector.unchanged=false;debug.renderGuidedCollection();assert(get('guided-collector-provenance').textContent.includes('verification failed'));data.collector.unchanged=savedCollector;debug.renderGuidedCollection();
  }else{assert.equal(get('guided-collection-panel').hidden,true);assert.equal(get('guided-collection-replay-panel').hidden,true);}
  if(bankStudy==='logged_graph'){
   assert.equal(get('banks-graph-panel').hidden,false);
   const preparation=data.logged_graph.per_bank.find(r=>r.bank_id===bank.bank_id),fit=data.fit_diagnostics,fr=fit.per_seed.filter(r=>r.bank_id===bank.bank_id),fmt=v=>v.toPrecision(4);
   for(const key of ['observed_edges','supported_states','graph_preparation_successor_backups','terminal_edges'])assert(get('banks-graph-preparation').innerHTML.includes(preparation[key].toLocaleString()));
   assert(get('banks-graph-preparation').innerHTML.includes(fmt(preparation.bellman_residual_max)));
   for(const key of ['inference_state_rows','observed_edge_comparisons'])assert(get('banks-graph-preparation-note').textContent.includes(fit[key].toLocaleString()));
   assert.equal((get('banks-graph-fit-table').innerHTML.match(/<tr>/g)||[]).length,fr.length+1);
   for(const r of fr){for(const key of ['states','observed_edges'])assert(get('banks-graph-fit-table').innerHTML.includes(`<td>${r[key].toLocaleString()}</td>`));for(const key of ['state_mean_abs_error','state_mean_squared_error','edge_mean_abs_error','edge_max_abs_error','mean_graph_regret','max_graph_regret'])assert(get('banks-graph-fit-table').innerHTML.includes(`<td>${fmt(r[key])}</td>`));assert(get('banks-graph-fit-table').innerHTML.includes(`${(100*r.unrestricted_argmax_outside_logged_fraction).toFixed(1)}%`));}
   for(const condition of conditions){const cr=fr.filter(r=>r.condition===condition),states=cr.reduce((sum,r)=>sum+r.states,0);for(const key of ['state_mean_abs_error','mean_graph_regret'])assert(get('banks-graph-fit-cards').innerHTML.includes(fmt(cr.reduce((sum,r)=>sum+r.states*r[key],0)/states)));}
   assert(get('banks-graph-fit-note').textContent.includes('No full-world Q*'));
   assert(get('banks-graph-fit-note').textContent.includes('1e-6 absolute'));
   assert(get('banks-action-query-note').textContent.includes('zero neural successor queries'));
   const savedFit=data.fit_diagnostics,savedGraph=data.logged_graph;delete data.fit_diagnostics;delete data.logged_graph;get('banks-selected-bank').listeners.change();assert(get('banks-graph-fit-cards').innerHTML.includes('—'));assert.equal((get('banks-graph-fit-table').innerHTML.match(/<tr>/g)||[]).length,1);assert(get('banks-graph-preparation').innerHTML.includes('—'));data.fit_diagnostics=savedFit;data.logged_graph=savedGraph;get('banks-selected-bank').listeners.change();
   for(const r of data.action_exposure.per_seed.filter(r=>r.bank_id===bank.bank_id&&r.condition==='logged_graph')){assert.equal(r.nonterminal_target_queries,0);assert(r.nonterminal_action_targets>0);const control=data.action_exposure.per_seed.find(c=>c.bank_id===bank.bank_id&&c.seed===r.seed&&c.condition==='constrained_bootstrap');if(control.updates===r.updates){assert.equal(r.action_target_presentations,control.action_target_presentations);assert.equal(r.nonterminal_action_targets,control.nonterminal_action_targets);}}
  }else assert.equal(get('banks-graph-panel').hidden,true);
  if(bankStudy==='constrained_bootstrap'){
   assert.equal(get('banks-bootstrap-panel').hidden,false);
   const diagnostics=data.bootstrap_diagnostics,dr=diagnostics.per_seed.filter(r=>r.bank_id===bank.bank_id),queries=dr.reduce((s,r)=>s+r.query_count,0),outside=dr.reduce((s,r)=>s+r.outside_count,0);
   assert(get('banks-bootstrap-cards').innerHTML.includes(`${outside.toLocaleString()} / ${queries.toLocaleString()}`));
   for(const r of dr){assert(get('banks-bootstrap-table').innerHTML.includes(`<td>${r.query_count.toLocaleString()}</td>`));assert(get('banks-bootstrap-table').innerHTML.includes(`${r.target_delta_min>0?'+':''}${r.target_delta_min.toFixed(3)} to ${r.target_delta_max>0?'+':''}${r.target_delta_max.toFixed(3)}`));}
   for(const [field,label] of [['positive_target_deltas','positive'],['negative_target_deltas','negative'],['zero_target_deltas','zero']])assert(get('banks-bootstrap-signs').textContent.includes(`${dr.reduce((sum,r)=>sum+r[field],0).toLocaleString()} ${label}`));
   assert(get('banks-bootstrap-note').textContent.includes('positive, negative, or zero'));assert(get('banks-bootstrap-note').textContent.includes('same weights'));
   const windows=diagnostics.windows.filter(r=>r.bank_id===bank.bank_id);assert.equal((get('banks-bootstrap-activation-chart').innerHTML.match(/<circle /g)||[]).length,windows.length);assert.equal((get('banks-bootstrap-delta-chart').innerHTML.match(/<circle /g)||[]).length,windows.length);
   if(windows.length){const saved=diagnostics.windows,template=windows[0];diagnostics.windows=[{...template,checkpoint:1,target_delta_mean:-.1},{...template,checkpoint:2,target_delta_mean:.1}];get('banks-selected-bank').listeners.change();const cy=[...get('banks-bootstrap-delta-chart').innerHTML.matchAll(/cy="([^"]+)"/g)].map(m=>Number(m[1]));assert.equal(cy.length,2);assert(cy.every(y=>y>=18&&y<=220),'Signed target deltas remain within plot bounds');assert(cy[0]>cy[1],'Negative target delta plots below positive delta');diagnostics.windows=saved;get('banks-selected-bank').listeners.change();}
   const ar=data.action_exposure.per_seed.filter(r=>r.bank_id===bank.bank_id);for(const r of ar.filter(r=>r.condition==='constrained_bootstrap')){const control=ar.find(a=>a.condition==='recorded_actions'&&a.seed===r.seed);if(control.updates===r.updates){assert.equal(r.action_target_presentations,control.action_target_presentations);assert.equal(r.nonterminal_target_queries,control.nonterminal_target_queries);}}
  }else assert.equal(get('banks-bootstrap-panel').hidden,true);
  if(['recorded_actions','constrained_bootstrap','logged_graph'].includes(bankStudy)){
   ...
   for(const r of targets){
    ...
    for(const key of ['terminal_action_targets','nonterminal_action_targets','nonterminal_target_queries','outside_support_target_queries'])assert(get('banks-action-query-table').innerHTML.includes(`<td>${r[key].toLocaleString()}</td>`));
   }
  ...
  if(['map_replay','within_map','recorded_actions','constrained_bootstrap','logged_graph','guided_collection'].includes(bankStudy)){
   assert.equal(get('banks-exposure-panel').hidden,false);if(bankStudy==='map_replay')assert(get('banks-intro-copy').textContent.includes('64 distinct maps'));else if(bankStudy==='guided_collection'){assert(get('banks-intro-copy').textContent.includes('complete collection episodes'));assert(get('banks-intro-copy').textContent.includes('not online RL'));assert(!get('banks-method').textContent.includes('No new states are collected'));}else if(bankStudy==='logged_graph'){assert(get('banks-intro-copy').textContent.includes('not full-world optimal values'));assert(get('banks-intro-copy').textContent.includes('All treatment fits finish'));assert(get('banks-method').textContent.includes('constrained-bootstrap archive'));}else if(bankStudy==='constrained_bootstrap'){assert(get('banks-intro-copy').textContent.includes('During training only'));assert(get('banks-intro-copy').textContent.includes('all four actions available'));assert(!get('banks-summary-table').innerHTML.includes('Collected'));assert(get('banks-method').textContent.includes('recorded-actions archive'));}else if(bankStudy==='recorded_actions'){assert(get('banks-intro-copy').textContent.includes('fewer supervised action targets'));}else{assert(get('banks-intro-copy').textContent.includes('within-batch map diversity'));assert(get('banks-intro-copy').textContent.includes('included states'));}if(bankStudy!=='guided_collection')assert(get('banks-intro-copy').textContent.includes('reevaluated'));
   if(bankStudy!=='guided_collection')assert(get('banks-composition-caption').textContent.includes('archived collection cost'));assert(!get('banks-summary-table').innerHTML.includes('Uniform'));
   if(bankStudy==='within_map'){
    ...
    if(Number.isFinite(data.run.per_map_subset_draws))assert(get('banks-method').textContent.includes(`${data.run.per_map_subset_draws.toLocaleString()} per-map subset draws`));
   }

   for(const seed of data.protocol.seeds){
    get('banks-exposure-seed').value=String(seed);get('banks-exposure-seed').listeners.change();
    const rows=data.exposure.per_map.filter(r=>r.bank_id===bank.bank_id && r.seed===seed);
    assert.equal((get('banks-exposure-chart').innerHTML.match(/<circle /g)||[]).length,rows.length);
    const lastMap=rows.at(-1).map_seed;get('banks-exposure-map').value=String(lastMap);get('banks-exposure-map').listeners.change();assert(get('banks-exposure-selected').textContent.includes(`Map ${lastMap.toLocaleString()}`));assert.equal((get('banks-exposure-chart').innerHTML.match(/r="5"/g)||[]).length,2);
    for(const r of rows)assert(get('banks-exposure-chart').innerHTML.includes(`Map ${r.map_seed.toLocaleString()}: ${r.presentations.toLocaleString()} presentations`));
    for(const condition of conditions){const cr=rows.filter(r=>r.condition===condition);assert(get('banks-exposure-totals').textContent.includes(`${cr[0].updates.toLocaleString()} updates`));assert(get('banks-exposure-totals').textContent.includes(`${cr.reduce((sum,r)=>sum+r.presentations,0).toLocaleString()} state presentations`));}
    for(const r of data.exposure.per_clock.filter(r=>r.bank_id===bank.bank_id && r.seed===seed))assert(get('banks-exposure-table').innerHTML.includes(`<td>${r.presentations.toLocaleString()}</td>`));
    assert(get('banks-exposure-totals').textContent.includes('archived baseline'));assert(get('banks-exposure-totals').textContent.includes('new treatment'));
    if(data.protocol.smoke)assert(get('banks-exposure-totals').textContent.includes('Smoke budgets differ'));
    if(bankStudy==='within_map'){
     assert.equal(get('banks-quota-check').hidden,false);assert(get('banks-quota-check').textContent.includes(`${bank.quotas.by_map.length} / ${bank.quotas.by_map.length} maps match`));
     assert(get('banks-quota-check').textContent.includes('identical ordered map/local-index streams and counts'));
     if(data.protocol.smoke)assert(get('banks-quota-check').textContent.includes('baseline-prefix'));
     const check=data.provenance.paired_map_sampling_consistency.find(r=>r.bank_id===bank.bank_id && r.seed===seed),savedFlag=check.map_digest_identical;
     check.map_digest_identical=false;get('banks-exposure-seed').listeners.change();assert(get('banks-quota-check').textContent.includes('not verified'));check.map_digest_identical=savedFlag;get('banks-exposure-seed').listeners.change();

     for(const q of bank.quotas.by_map){assert.equal(q.collected_states,q.treatment_states);const mr=rows.filter(r=>r.map_seed===q.map_seed);assert(mr.every(r=>r.supported_states===q.collected_states));}
     const ar=rows.filter(r=>r.condition===conditions[0]),br=rows.filter(r=>r.condition===conditions[1]);if(ar[0].updates===br[0].updates)for(const r of ar)assert.equal(r.presentations,br.find(v=>v.map_seed===r.map_seed).presentations);
    }

    for(const r of data.exposure.summaries.filter(r=>r.bank_id===bank.bank_id && r.seed===seed)){
     assert(get('banks-exposure-distribution').innerHTML.includes(`${r.successor_queries.outside_support_queries.toLocaleString()} / ${r.successor_queries.nonterminal_queries.toLocaleString()}`));
     for(const c of Object.values(r.category_exposure))assert(get('banks-exposure-categories').innerHTML.includes(`<td>${c.presentations.toLocaleString()}</td>`));
    }

   }
  }else assert.equal(get('banks-exposure-panel').hidden,true);
 ...
 if(bankStudy==='guided_collection'){
  let collectionRecordings=0;
  for(const row of data.collection_trajectories){get('banks-selected-bank').value=String(row.bank_id);get('banks-selected-bank').listeners.change();get('guided-collection-replay-map').value=String(row.map_seed);get('guided-collection-replay-map').listeners.change();get('guided-collection-replay-condition').value=row.condition;get('guided-collection-replay-condition').listeners.change();get('guided-collection-replay-slot').value=String(row.repetition);get('guided-collection-replay-slot').listeners.change();assert.equal(debug.guidedSelected,row);assert(get('guided-collection-world-caption').textContent.startsWith('Rule A'));assert(get('guided-collection-replay-explanation').textContent.includes('complete collection episode'));if(row.policy_component==='guided')assert(get('guided-collection-step-diagnostics').innerHTML.includes('Learned Q'));debug.guidedFrame=row.steps.length;debug.drawGuidedCollectionWorld();assert(get('guided-collection-replay-step').textContent.includes(String(row.steps.length)));collectionRecordings++;}
  for(const r of data.collection_trajectories.filter(r=>r.condition===conditions[1]&&r.repetition===0)){const old=data.collection_trajectories.find(v=>v.bank_id===r.bank_id&&v.condition===conditions[0]&&v.map_seed===r.map_seed&&v.repetition===0);assert.deepEqual(r.steps,old.steps,'Predetermined shared-random collection recordings match exactly');}
  get('guided-collection-replay-reset').listeners.click();get('guided-collection-replay-play').listeners.click();assert.equal(intervals.size,1);[...intervals.values()][0]();get('banks-replay-play').listeners.click();assert.equal(intervals.size,1,'Fresh playback stops collection playback');get('guided-collection-replay-play').listeners.click();assert.equal(intervals.size,1,'Collection playback stops fresh playback');debug.activateTab('adaptation');assert.equal(intervals.size,0);debug.activateTab('coverage');get('guided-collection-replay-scrub').listeners.input({target:{value:'0'}});
  console.log(`Guided collection: all ${collectionRecordings} collection recordings, complete-episode costs, frozen collector prior cost, random-slot identity, actual action-target counts, route ceilings and exclusive playback passed.`);
 }
 let recordings=0;
 ...
 get('banks-replay-reset').listeners.click();get('banks-replay-play').listeners.click();get('coverage-study').value='panel_evaluation';get('coverage-study').listeners.change();assert.equal(intervals.size,0);assert.equal(get('banks-view').hidden,true);get('coverage-study').value=bankStudy;get('coverage-study').listeners.change();assert.equal(get('banks-view').hidden,false);assert.equal(debug.banksSelected.bank_id,records[0].bank_id);assert.equal(debug.banksSelected.panel,panels[0]);
 if(bankStudy==='guided_collection'){get('guided-collection-replay-play').listeners.click();assert.equal(intervals.size,1);}
 await get('refresh').listeners.click();assert.equal(get('banks-content').hidden,false);if(bankStudy==='guided_collection'){assert.equal(intervals.size,0);assert.equal(debug.guidedSelected.map_seed,Math.min(...data.collection_trajectories.map(r=>r.map_seed)));assert.equal(debug.guidedSelected.repetition,8,'Collection slot 8 preselected before results');}
 console.log(`${bankStudy}: ${records.length} bank pairs, ${panels.length} panels, all ${recordings} recordings, individual/equal-bank effects, bank composition, both action modes, shared references, ineligible/empty branches and playback/study-switch/refresh passed.`);
}else{assert.equal(get('banks-view').hidden,false);assert.equal(get('banks-content').hidden,true);assert.equal(get('banks-empty').hidden,false);assert.equal(get('robustness-view').hidden,true);console.log(`Missing ${bankStudy} stays empty; no previous study substituted.`);}
}
