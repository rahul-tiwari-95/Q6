...
if(debug.familiar?.aggregate?.length){
 ...
 const pp=v=>Number.isFinite(v)?`${v>0?'+':''}${(100*v).toFixed(1)} pp`:'—';assert.deepEqual([...get('familiar-effects').innerHTML.matchAll(/<strong[^>]*>([^<]*)<\/strong>/g)].map(m=>m[1]),expected.map(pp));
