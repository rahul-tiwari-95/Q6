...
def jonckheere_terpstra_test(groups: list, alternative: str, n_permutations: int,
    ...
    Returns (J, p_value). J is always computed in "natural order" (so it's
    comparable across calls); the p-value is one-sided in the direction
    `alternative` names.
    """
    ...
    for p in range(n_permutations):
        permuted[p] = statistic(rng.permutation(pooled))
