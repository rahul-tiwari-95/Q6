...
Directly tests the open question flagged in results/provenance_test_v1.md:
"whether a learned policy would discover the unique-origin distinction on
its own, given only raw message features as input." Two learner configs are
...
class TabularQMandateLearner:
    ...
    def __call__(self, state: WorldState, rng: np.random.Generator) -> bool:
        ...
        # Found the hard way, by actually running this: a CONSTANT alpha
        # never stabilizes here -- rare bins keep getting overwritten by
        # noisy single samples indefinitely, and 4x more training data made
        # one bin's action preference FLIP rather than settle. Pure 1/n
        # decay (textbook Robbins-Monro) fixed the instability but
        # overshot: with sparse, mostly-zero per-tick rewards, Q-values in
        # rarely-visited bins decayed toward ~0 and lost any real
        # preference between actions. The floor keeps enough sensitivity to
        # rare-but-important reward signal while still converging -- see
        # results/learning_citizen_v1.md for all three variants' numbers,
        # kept rather than only reporting the one that worked.
        if self.training and self._last_bin is not None and state.events:
            ...
            best_next_q = max(self._q(current_bin, a) for a in (True, False))
