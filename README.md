...
**Latest finding:** the same Double DQN procedure reaches **80.21% fresh-layout success with exhaustive states versus 68.23% with states collected through random trajectories**. Collected training reaches 97.14% on familiar starts, yet loses 11.98 percentage points on fresh maps and produces fewer efficient successes. All exhaustive seeds pass the declared fresh gate; two collected seeds miss it. The next control holds the number of unique states fixed to test trajectory-induced composition. Read the [coverage result](docs/experiments/coverage_results_v1.md). Both conditions retain privileged four-action access; this is offline evidence, not online-RL competence.
...
python -m q6.coverage --output /tmp/q6-coverage-smoke \
  --protocol-file docs/experiments/coverage_protocol_v1.md --smoke
```
...
| --- | --- | --- |
| **Competence → adaptation — primary** | With DDQN fixed, how does collected state support affect learning and transfer? | [Current protocol](docs/experiments/coverage_protocol_v1.md), [results](docs/experiments/coverage_results_v1.md), [`q6/`](q6/) |
...
Reproduce the current state-coverage comparison in a fresh directory:

```bash
# Match the main run's core dependencies in Python 3.12.
python -m pip install 'torch==2.8.0' 'numpy==2.0.2'
PYTHONHASHSEED=0 OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 nice -n 10 python -m q6.coverage \
  --output experiments/coverage/my-reproduction \
  --protocol-file docs/experiments/coverage_protocol_v1.md
```

The [captured environment](experiments/coverage/pilot_v1/environment.txt) records the remaining versions. The runner checks that its exhaustive arm reproduces the shipped fixed-target DDQN archive. It runs sequentially on one CPU thread with a sampled 4 GiB process-RSS stop guard; the main study took 157 seconds and peaked at 0.59 GiB. To display a reproduction locally, add `--dashboard dashboard/data/coverage.json`; this replaces the viewer's current data, so preserve the original artifact.
...
Open **[the current lab](http://127.0.0.1:8080/dashboard/lab.html#coverage)** and select **Experience coverage**. Start with the 256-layout coverage heatmap and remaining-time bars, then compare fresh success, efficient success, episode length and paired differences. The support diagram distinguishes states sampled for training from successor values queried for targets. Replays retain the visible rule and learned/exact pre-action values. The first fresh map was selected before outcomes and illustrates variability; use full-panel measurements to assess the comparison.

**Fixed-data targets** preserves the exact-versus-DDQN comparison. **Exact targets**, **Learn one world**, **Adaptation** and **Evidence** retain the earlier tracks and their own evaluation panels. The [historical registry](http://127.0.0.1:8080/dashboard/index.html) preserves older run summaries. To index legacy training runs available on your machine, run `python dashboard/scan.py` before starting the server.
...
- [Coverage validation](docs/validation/coverage-v1.md), [fixed-target validation](docs/validation/fixed-targets-v1.md), [supervised validation](docs/validation/supervised-v1.md), [competence validation](docs/validation/competence-v1.md) and [earlier preview validation](docs/validation/2026-09-06.md): tests, CI, artifact audits and browser coverage.
