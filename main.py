"""
Main Training Script - Curriculum Learning for Kṛṣṇa
=====================================================
Version: 0.5 (Strategic Curriculum Learning with Progressive Hunter)
Previous Versions: 0.4 (17.5K episodes), 0.3 (10K episodes), 0.1 (archived)

PHILOSOPHY:
-----------
V0.5 implements a fundamentally different curriculum approach based on v0.4 analysis.

THE V0.4 DISCOVERY:
-------------------
V0.4 training revealed a critical insight:
- Phases 1-3: EXCELLENT (60-70% win rates)
- Phase 4: STRUGGLING (17.98% win rate, no improvement over 10K episodes)
- Root Cause: Hunter introduced too late and too strong = difficulty shock

THE SWIMMING ANALOGY:
---------------------
V0.4 Approach (FAILED):
  "Learn to swim in calm pool → then thrown into ocean with sharks"
  
V0.5 Approach (BETTER):
  "Learn with shark behind glass → slow shark → medium shark → fast shark"
  
KEY INSIGHT: Don't hide the final challenge until the end. Introduce it early
at low difficulty, then gradually increase. Builds challenge-specific skills
from day 1 instead of hoping general skills transfer.

CURRICULUM LEARNING APPROACH V0.5:
-----------------------------------
Progressive Hunter System - Agent faces Hunter from episode 1, but difficulty increases:

Phase 1 (Episodes 1-3000):     Random Hunter       → Learn "Hunter exists"
Phase 2 (Episodes 3001-6000):  Greedy Hunter       → Learn "I can use walls"
Phase 3 (Episodes 6001-9000):  Smart Greedy        → Learn "Must actively evade"
Phase 4 (Episodes 9001-15000): A* Hunter           → Learn "Advanced strategies"

HUNTER PROGRESSION DETAILS:
----------------------------
1. Random Hunter (Phase 1):
   - Moves randomly, ignores Krishna
   - Catches by luck only
   - Very low pressure, agent learns fundamentals
   
2. Greedy Hunter (Phase 2):
   - Moves toward Krishna (direct path)
   - Can be trapped behind walls
   - Medium pressure, evasion tactics emerge
   
3. Smart Greedy Hunter (Phase 3):
   - Like Greedy but avoids getting stuck
   - Persistent but still no pathfinding
   - High pressure, strategic positioning required
   
4. A* Hunter (Phase 4):
   - Optimal pathfinding, cannot be trapped
   - Maximum pressure, advanced strategies needed
   - Same as v0.4's Phase 4, but agent is prepared

KEY IMPROVEMENTS IN V0.5:
-------------------------
1. PROGRESSIVE HUNTER: Hunter present from episode 1, difficulty scales
   - Builds Hunter-specific skills progressively
   - No sudden difficulty shocks
   - Transfer learning across phases

2. COMPOSITE SUCCESS SCORING: Beyond binary win/loss (0/1)
   ...
   - Success Tiers: S/A/B/C/D/F ranks

3. PHASE-ADAPTIVE EPSILON RESET: Fresh exploration at transitions
   - Phase 1: ε=1.0 (full exploration)
   - Phase 2: ε=0.5 (balanced)
   - Phase 3: ε=0.3 (mostly exploit)
   - Phase 4: ε=0.2 (strategic exploration)

4. REDUCED CATCH PENALTY: -10 (vs -20 in v0.4)
   - Encourages calculated risk-taking
   - Agent learns "collect while avoiding" not "just avoid"

5. OPTIMIZED CURRICULUM: [3000, 3000, 3000, 9000] = 15K episodes
   - Stronger foundation per phase
   - Faster iteration for development
   - Can scale to 50K for production

6. ENHANCED TERMINAL OUTPUT: Beautiful real-time monitoring
...
----------------------
V0.4 Baseline:
  - Overall: 37.79% win rate
  - Phase 4: 17.98% win rate (PROBLEM)
  
V0.5 Target:
  - Overall Success: 0.55-0.65 (composite score)
  - Phase 4 Success: 0.40-0.55 (IMPROVEMENT expected)
  - Tier Distribution: Normal distribution centered on C/B ranks

This accepts common sense and logic as its friend. Progressive difficulty
is how humans learn. Why should AI be different?
"""
...
from utils.terminal_display import TerminalDisplay  # V0.5 NEW
...
def create_training_directories() -> Tuple[str, str, str]:
    ...
    print(f"{'='*80}")
    print(f"║  📁 Checkpoints: {checkpoint_dir:<54} ║")
    print(f"║  📋 Logs:        {log_dir:<54} ║")
    print(f"║  📊 Plots:       {plot_dir:<54} ║")
    print(f"{'='*80}\n")
...
def plot_scores(scores: List[float], 
    ...
    if phase_boundaries:
        for i, boundary in enumerate(phase_boundaries):
            plt.axvline(x=boundary, color='green', linestyle='--', alpha=0.7, linewidth=2)
            plt.text(boundary, plt.ylim()[1]*0.95, f'Phase {i+1}→{i+2}', 
                    rotation=90, va='top', fontsize=10, color='green', fontweight='bold')
    
    plt.xlabel('Episode', fontsize=12)
    plt.ylabel('Score', fontsize=12)
    plt.title(f'Curriculum Learning Progress (Episodes 1-{episode})', fontsize=14, fontweight='bold')
...
def generate_comprehensive_analysis(scores: List[float],
    ...
    print(f"\n{'╔'+'═'*78+'╗'}")
    print(f"║{'GENERATING COMPREHENSIVE POST-TRAINING ANALYSIS':^78}║")
    print(f"{'╚'+'═'*78+'╝'}\n")
...
def print_hyperparameters(agent: DQNAgent) -> None:
    """Print agent hyperparameters with beautiful formatting."""
    print(f"\n{'╔'+'═'*78+'╗'}")
    print(f"║{'AGENT HYPERPARAMETERS':^78}║")
    print(f"{'╠'+'═'*78+'╣'}")
    print(f"║  Learning Rate (α):       {agent.learning_rate:<47} ║")
    print(f"║  Discount Factor (γ):     {agent.gamma:<47} ║")
    print(f"║  Batch Size:              {agent.batch_size:<47} ║")
    print(f"║  Replay Buffer Size:      {len(agent.memory)}/{agent.memory.buffer.maxlen:<40} ║")
    print(f"║  Target Update (τ):       {agent.tau:<47} ║")
    print(f"║  Network Architecture:    {agent.state_size} → {agent.hidden_sizes} → {agent.action_size:<26} ║")
    print(f"║  Device:                  {str(agent.device):<47} ║")
    print(f"║{'':<78}║")
    print(f"║  {'EXPLORATION STRATEGY (IMPROVED)':^76} ║")
    print(f"║  Initial Epsilon:         {agent.epsilon:<47} ║")
    print(f"║  Minimum Epsilon:         {agent.epsilon_min} (was 0.01, now 5x more exploration){'':<20} ║")
    print(f"║  Decay Rate:              {agent.epsilon_decay} (was 0.995, now 10x slower decay){'':<16} ║")
    print(f"{'╚'+'═'*78+'╝'}\n")


def print_environment_info(env) -> None:
    """Print environment configuration with beautiful formatting."""
    print(f"\n{'╔'+'═'*78+'╗'}")
    print(f"║{'ENVIRONMENT CONFIGURATION':^78}║")
    print(f"{'╠'+'═'*78+'╣'}")
    print(f"║  Grid Size:               25x25{'':<49} ║")
    print(f"║  State Space Size:        {env.state_size} (flattened grid){'':<39} ║")
    print(f"║  Actions:                 4 (Up, Down, Left, Right){'':<29} ║")
    print(f"║{'':<78}║")
    print(f"║  {'REWARD STRUCTURE':^76} ║")
    print(f"║    Collect Pellet:        +50{'':<48} ║")
    print(f"║    Get Caught:            -20{'':<48} ║")
    print(f"║    Hit Wall:              -5{'':<49} ║")
    print(f"║    Per Step Penalty:      -0.01 (CRITICAL FIX: was -0.1){'':<22} ║")
    print(f"║    Win (Score >= 200):    +100{'':<47} ║")
    print(f"║{'':<78}║")
    print(f"║  Win Condition:           Collect 4 pellets (4 × 50 = 200 points){'':<13} ║")
    print(f"║  Loss Condition:          Lose all 3 lives{'':<38} ║")
    print(f"{'╚'+'═'*78+'╝'}\n")


def print_curriculum_phase(phase: int, episode: int, total_episodes: int) -> None:
    """Print curriculum learning phase information with beautiful formatting."""
    phases = {
        1: {
            'name': 'EASY MODE',
            'emoji': '🟢',
        ...
        4: {
            'name': 'EXPERT MODE - FULL GAME',
            'emoji': '🔴',
            'enemies': '2 Greedy + 2 Patrollers + 1 Hunter (A* pathfinding)',
            'description': 'Face intelligent A* pathfinding enemy - THE REAL CHALLENGE',
            'episodes': '4501-10000 (EXTENDED - 5500 episodes for pattern emergence)'
        }
    }
    
    info = phases.get(phase, phases[1])
    print(f"\n{'╔'+'═'*78+'╗'}")
    print(f"║{info['emoji']} {'CURRICULUM PHASE ' + str(phase) + ': ' + info['name']:^74}{info['emoji']}║")
    print(f"{'╠'+'═'*78+'╣'}")
    print(f"║  Episodes:  {info['episodes']:<63} ║")
    print(f"║  Progress:  {episode}/{total_episodes} ({episode/total_episodes*100:.1f}% complete){'':<37} ║")
    print(f"║  Enemies:   {info['enemies']:<63} ║")
    print(f"║  Goal:      {info['description']:<63} ║")
    print(f"{'╚'+'═'*78+'╝'}\n")


def train(n_episodes: int = 15000,
          max_steps: int = 1000,
          verbose_every: int = 10,
          phase_episodes: List[int] = [3000, 3000, 3000, 9000]) -> None:
    """
    Main curriculum training function - V0.5 STRATEGIC CURRICULUM.
    
    V0.5 REVOLUTIONARY CHANGE:
    --------------------------
    Progressive Hunter System - Hunter present from episode 1, difficulty scales:
    - Phase 1 (1-3000): Random Hunter (learns fundamentals with low pressure)
    - Phase 2 (3001-6000): Greedy Hunter (learns evasion tactics)
    - Phase 3 (6001-9000): Smart Greedy (learns strategic positioning)
    - Phase 4 (9001-15000): A* Hunter (masters advanced strategies)
    
    KEY INSIGHT from v0.4:
    ----------------------
    Don't hide challenge until end. Introduce early at low difficulty, scale up.
    Builds challenge-specific skills from day 1 vs hoping general skills transfer.
    
    Args:
        n_episodes (int): Total number of episodes (default: 15000)
        ...
        phase_episodes (list): Episodes per phase [phase1, phase2, phase3, phase4]
                               Default: [3000, 3000, 3000, 9000] = 15,000 total
    """
    
    # V0.5: Print beautiful header
    TerminalDisplay.print_v05_header()
    ...
    # Enable Q-value logging for network analysis
    agent.enable_q_logging(log_dir)
    ...
    print(f"\n{'╔'+'═'*78+'╗'}")
    print(f"║{'CURRICULUM PHASES':^78}║")
    print(f"{'╠'+'═'*78+'╣'}")
    print(f"║  Phase 1 (Easy):    Episodes 1-{phase_boundaries[0]:<4} ({phase_episodes[0]} episodes){'':<29} ║")
    print(f"║  Phase 2 (Medium):  Episodes {phase_boundaries[0]+1}-{phase_boundaries[1]} ({phase_episodes[1]} episodes){'':<28} ║")
    print(f"║  Phase 3 (Hard):    Episodes {phase_boundaries[1]+1}-{phase_boundaries[2]} ({phase_episodes[2]} episodes){'':<28} ║")
    print(f"║  Phase 4 (Expert):  Episodes {phase_boundaries[2]+1}-{phase_boundaries[3]} ({phase_episodes[3]} episodes){'':<27} ║")
    print(f"║{'':<78}║")
    print(f"║  ⚠️  NOTE: Phase 4 is INTENTIONALLY LONGER ({phase_episodes[3]} episodes){'':<25} ║")
    print(f"║      Goal: See if deep patterns emerge with extended training{'':<17} ║")
    print(f"{'╚'+'═'*78+'╝'}\n")
    ...
    current_phase = 0  # V0.5 CHANGED: Start at 0 to detect first phase transition
    env = None
    
    # V0.5 NEW: Track phase-specific data for summaries
    phase_data = {1: [], 2: [], 3: [], 4: []}
    
    print(f"\n{'╔'+'═'*78+'╗'}")
    print(f"║{'🚀 STARTING CURRICULUM TRAINING':^78}║")
    print(f"{'╚'+'═'*78+'╝'}\n")

    # ===== MAIN TRAINING LOOP =====
    start_time = datetime.now()  # V0.5 NEW: Track total training time
    for i_episode in range(1, n_episodes + 1):
        
        # Determine difficulty level and curriculum phase based on episode
        if i_episode <= phase_boundaries[0]:
            difficulty = 1
            curriculum_phase = 1  # V0.5 NEW: Random Hunter
        elif i_episode <= phase_boundaries[1]:
            difficulty = 2
            curriculum_phase = 2  # V0.5 NEW: Greedy Hunter
        elif i_episode <= phase_boundaries[2]:
            difficulty = 3
            curriculum_phase = 3  # V0.5 NEW: Smart Greedy Hunter
        else:
            difficulty = 4
            curriculum_phase = 4  # V0.5 NEW: A* Hunter
        
        # Create new environment if phase changed
        if difficulty != current_phase:
            # Store previous phase data for summary
            if current_phase > 0 and phase_data[current_phase]:
                hunter_names = {1: "Random Hunter", 2: "Greedy Hunter", 3: "Smart Greedy Hunter", 4: "A* Hunter"}
                TerminalDisplay.print_phase_footer()
                TerminalDisplay.print_phase_summary(phase_data[current_phase], current_phase, hunter_names[current_phase])
            
            current_phase = difficulty
            
            # V0.5 NEW: Reset epsilon at phase transitions
            old_eps, new_eps = agent.reset_epsilon_for_phase(current_phase)
            if new_eps > old_eps:
                TerminalDisplay.print_epsilon_reset(old_eps, new_eps, current_phase)
            
            # Print phase header
            hunter_names = {1: "Random Hunter", 2: "Greedy Hunter", 3: "Smart Greedy Hunter", 4: "A* Hunter"}
            TerminalDisplay.print_phase_header(current_phase, i_episode, phase_boundaries[current_phase-1] if current_phase < 4 else n_episodes, hunter_names[current_phase])
            
            # Create environment with curriculum phase
            base_env = HunterGridworld(difficulty_level=current_phase, curriculum_phase=curriculum_phase)
        ...
        if env is None:
            hunter_names = {1: "Random Hunter", 2: "Greedy Hunter", 3: "Smart Greedy Hunter", 4: "A* Hunter"}
            TerminalDisplay.print_phase_header(1, i_episode, phase_boundaries[0], hunter_names[1])
            base_env = HunterGridworld(difficulty_level=1, curriculum_phase=1)
            env = EnvironmentMetricsWrapper(base_env)
            print_environment_info(base_env)
        ...
        for t in range(max_steps):
            ...
            # Store experience and learn (pass episode number for Q-logging)
            agent.step(state, action, reward, next_state, done or truncated, episode=i_episode)
            ...
            if (i_episode % verbose_every == 0 and t < 5):
                print(f"  │ Step {t+1:3d}: {action_to_name(action)} │ "
                      f"Pos={str(info.get('krishna_position', 'N/A')):12} │ "
                      f"Reward={reward:7.2f} │ Score={info.get('score', 0):4.0f} │ "
        ...
        # Check win condition using new generalized system (pellets-based, not score-based)
        episode_won = env.env.check_win_condition()
        
        # V0.5 NEW: Get success scoring info from environment
        info_final = base_env._get_info()
        ...
        episode_data = {
            ...
            'phase': current_phase,
            'score': score,
            'total_reward': score,  # V0.5: Alias for compatibility
            'pellets_collected': metrics['pellets_collected'],
            'times_caught': metrics['times_caught'],
            'walls_hit': metrics['walls_hit'],
            'steps_taken': metrics['total_steps'],
            'steps': metrics['total_steps'],  # V0.5: Alias
            'epsilon': agent.epsilon,
            'won': episode_won,  # Task-based win condition
            'success_score': info_final.get('success_score', 0.0),  # V0.5 NEW
            'success_tier': info_final.get('success_tier', 'F'),  # V0.5 NEW
            'success_tier_emoji': info_final.get('success_tier_emoji', '❌'),  # V0.5 NEW
            'success_tier_label': info_final.get('success_tier_label', 'FAILURE')  # V0.5 NEW
        }
    ...
    # V0.5 NEW: Print final phase summary
    TerminalDisplay.print_phase_footer()
    hunter_names = {1: "Random Hunter", 2: "Greedy Hunter", 3: "Smart Greedy Hunter", 4: "A* Hunter"}
    if phase_data[current_phase]:
        TerminalDisplay.print_phase_summary(phase_data[current_phase], current_phase, hunter_names[current_phase])
    ...
    print(f"\n{'╔'+'═'*78+'╗'}")
    print(f"║{'STARTING COMPREHENSIVE POST-TRAINING ANALYSIS':^78}║")
    print(f"{'╚'+'═'*78+'╝'}")
    print("\nThis will generate detailed visualizations to understand:")
    ...
    # V0.5 NEW: Beautiful final summary
    TerminalDisplay.print_final_summary(episodes_data, training_time, os.path.join(log_dir, '..'))
...
if __name__ == '__main__':
    ...
    # VERSION HISTORY:
    # ----------------
    # v0.1 (hunter_version_1.py): FAILED - All enemies from start, agent learned nothing
    # v0.3: 10K episodes [1500, 1500, 1500, 5500], basic curriculum
    # v0.4: 17.5K episodes [2500, 2500, 2500, 10000], slower epsilon, task-based wins
    #       Result: 37.79% overall win rate, BUT Phase 4 only 17.98%
    # v0.5: 15K episodes [3000, 3000, 3000, 9000], PROGRESSIVE HUNTER SYSTEM
    #
    # V0.5 KEY INNOVATION:
    # --------------------
    # Progressive Hunter: Hunter present from episode 1, difficulty scales
    # - Phase 1: Random Hunter (low pressure, learn fundamentals)
    # - Phase 2: Greedy Hunter (medium pressure, learn evasion)
    # - Phase 3: Smart Greedy (high pressure, learn positioning)
    # - Phase 4: A* Hunter (max pressure, master strategies)
    #
    # Additional v0.5 Features:
    # - Composite success scoring (S/A/B/C/D/F tiers)
    # - Phase-adaptive epsilon reset (fresh exploration per phase)
    # - Reduced catch penalty (-10 vs -20)
    # - Enhanced terminal output (beautiful real-time monitoring)
    #
    # Expected Improvement:
    # - Phase 4 success: 17.98% → 40-55% (target)
    # - Overall: Better learning curve, smoother progression
    
    print("\n" + "═"*80)
    print("  Starting V0.5 Training - Progressive Hunter Curriculum")
    print("  Expected Duration: ~60 minutes (15,000 episodes)")
    print("═"*80 + "\n")
