...
const coverageStudies={coverage:null,equal_support:null,panel_evaluation:null,bank_replication:null,map_replay:null,within_map:null,recorded_actions:null,constrained_bootstrap:null};
...
const coverageColors={exhaustive:'#a6e5c1',collected_unique:'#98b8d1',uniform_subset:'#e7b985',map_balanced:'#a6e5c1',within_map_uniform:'#c4aadf',recorded_actions:'#d6ba83',constrained_bootstrap:'#9bd1c9'};
...
const banksHasRecordedSupervision=()=>banksIsRecorded() || banksIsConstrained();
...
const banksControlShort=()=>banksIsConstrained()?'Recorded':'Collected';
const banksLabel=condition=>banksHasArchivedControl() && condition===banksConditions()[0]?(banksIsConstrained()?'Recorded actions · archived baseline':banksIsRecorded()?'All-action collected · archived baseline':'Collected replay · archived baseline'):banksHasArchivedControl() && condition===banksConditions()[1]?`${coverageLabel(condition)} · new fit`:coverageLabel(condition);
...
function renderBanksStudyCopy(){
  ...
  $('banks-exposure-eyebrow').textContent=constrained?'SAME STATE EXPOSURE / SAME RECORDED ACTION TARGET COUNTS':recorded?'SAME STATE EXPOSURE / FEWER ACTION TARGETS':within?'DIFFERENT INCLUDED STATES / MATCHED MAP EXPOSURE':'SAME INCLUDED STATES / DIFFERENT REPLAY EXPOSURE';
}
...
function renderBanks(){
  ...
  $('banks-method').textContent=`${number(run.wall_seconds)} seconds elapsed. ${mapReplay?`No new states are collected. Only ${constrained?'constrained-bootstrap':recorded?'recorded-action':within?'within-map uniform':'map-balanced'} treatments are fitted; baseline models and training exposure come from the ${constrained?'recorded-actions':'bank-replication'} archive. Both final policies are evaluated on this study’s new panels.`:'New collected banks and matching uniform subsets are fitted with the same learner setup.'} Only final policies are evaluated. Intermediate model snapshots and training losses are archived for reproduction; they are not intermediate evaluation results.`;
  ...
  $('banks-evidence-links').innerHTML=`<a href="data/${banksStudyKey()}.json" download>Download displayed data ↓</a>`+[['protocol_document','Study design'],['protocol','Saved protocol'],['report','Measured findings'],['evaluations','Raw final episodes'],['training',mapReplay?'New treatment loss log':'Training loss log'],['losses',mapReplay?'New treatment loss log':'Training loss log'],['exposure','Actual training exposure'],['bootstrap_diagnostics','Training target diagnostics'],['bootstrap_probes','Fixed training probes'],['banks','Bank composition'],['sampling','Sampling provenance'],['paired_differences','Paired differences'],['manifest','Artifact checksums']].flatMap(([key,label])=>{const path=study.artifacts?.[key];return typeof path==='string' && /^(docs|experiments)\/[a-zA-Z0-9_./-]+$/.test(path) && !path.split('/').includes('..')?[`<a href="../${escape(path)}">${label} ↗</a>`]:[];}).join('');
...
function renderBanksComparison(){
  ...
  renderBanksComposition();renderBanksExposure();renderBanksActions();renderBanksBootstrap();selectBanksTrajectory();
}
...
async function loadData() {
  ...
  const results=await Promise.allSettled([getData(adaptationPath),getData('data/provenance.json'),getData('data/competence.json'),getData('data/supervised.json'),getData('data/fixed_targets.json'),getData('data/coverage.json'),getData('data/equal_support.json'),getData('data/panel_evaluation.json'),getData('data/bank_replication.json'),getData('data/map_replay.json'),getData('data/within_map.json'),getData('data/recorded_actions.json'),getData('data/constrained_bootstrap.json')]);
  const errors=[];
  diagnostics=null;
