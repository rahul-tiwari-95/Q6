...
const coverageStudies={coverage:null,equal_support:null,panel_evaluation:null,bank_replication:null,map_replay:null,within_map:null};
...
const coverageColors={exhaustive:'#a6e5c1',collected_unique:'#98b8d1',uniform_subset:'#e7b985',map_balanced:'#a6e5c1',within_map_uniform:'#c4aadf'};
const coverageLabel=condition=>({exhaustive:'Exhaustive states',collected_unique:'Collected unique states',uniform_subset:'Uniform subset',map_balanced:'Map-balanced replay',within_map_uniform:'Within-map uniform states',shared:'Shared references'}[condition] || condition);
...
const banksHasArchivedControl=()=>banksIsMapReplay() || banksIsWithinMap();
const banksStudyKey=()=>banksIsWithinMap()?'within_map':banksIsMapReplay()?'map_replay':'bank_replication';
function banksData(){return coverageStudies[banksStudyKey()];}
const banksConditions=()=>['collected_unique',banksIsWithinMap()?'within_map_uniform':banksIsMapReplay()?'map_balanced':'uniform_subset'];
const banksDirection=()=>banksIsWithinMap()?'Within-map uniform minus collected':banksIsMapReplay()?'Map-balanced minus collected':'Uniform minus collected';
const banksTreatmentShort=()=>banksIsWithinMap()?'Within-map':banksIsMapReplay()?'Balanced':'Uniform';
const banksLabel=condition=>banksHasArchivedControl() && condition==='collected_unique'?'Collected replay · archived baseline':banksHasArchivedControl() && condition===banksConditions()[1]?`${coverageLabel(condition)} · new fit`:coverageLabel(condition);
...
function renderBanksStudyCopy(){
  ...
  $('banks-exposure-panel').hidden=!(map || within);
  $('banks-exposure-eyebrow').textContent=within?'DIFFERENT INCLUDED STATES / MATCHED MAP EXPOSURE':'SAME INCLUDED STATES / DIFFERENT REPLAY EXPOSURE';
}
...
function renderBanks(){
  ...
  $('banks-method').textContent=`${number(run.wall_seconds)} seconds elapsed. ${mapReplay?`No new states are collected. Only ${within?'within-map uniform':'map-balanced'} treatments are fitted; baseline models and training exposure come from the bank-replication archive. Both final policies are evaluated on this study’s new panels.`:'New collected banks and matching uniform subsets are fitted with the same learner setup.'} Only final policies are evaluated. Intermediate model snapshots and training losses are archived for reproduction; they are not intermediate evaluation results.`;
...
async function loadData() {
  ...
  const results=await Promise.allSettled([getData(adaptationPath),getData('data/provenance.json'),getData('data/competence.json'),getData('data/supervised.json'),getData('data/fixed_targets.json'),getData('data/coverage.json'),getData('data/equal_support.json'),getData('data/panel_evaluation.json'),getData('data/bank_replication.json'),getData('data/map_replay.json'),getData('data/within_map.json')]);
  const errors=[];
  diagnostics=null;
  ...
  if(errors.length) $('load-status').textContent=errors.join(' · ');
    ...
    const loaded=[coverageStudies.within_map&&'Within-map states',coverageStudies.map_replay&&'Map-balanced replay',coverageStudies.bank_replication&&'Bank replications',coverageStudies.panel_evaluation&&'Panel robustness',coverageStudies.equal_support&&'Equal-size banks',coverageStudies.coverage&&'Experience coverage',fixed&&'Fixed-data targets',supervised&&'Exact targets',competence&&'Competence',adaptation&&'Adaptation',provenance&&'Provenance'].filter(Boolean);
