...
const coverageStudies={coverage:null,equal_support:null,panel_evaluation:null,bank_replication:null,map_replay:null};
...
const coverageColors={exhaustive:'#a6e5c1',collected_unique:'#98b8d1',uniform_subset:'#e7b985',map_balanced:'#a6e5c1'};
const coverageLabel=condition=>({exhaustive:'Exhaustive states',collected_unique:'Collected unique states',uniform_subset:'Uniform subset',map_balanced:'Map-balanced replay',shared:'Shared references'}[condition] || condition);
...
const banksStudyKey=()=>banksIsMapReplay()?'map_replay':'bank_replication';
function banksData(){return coverageStudies[banksStudyKey()];}
const banksConditions=()=>['collected_unique',banksIsMapReplay()?'map_balanced':'uniform_subset'];
const banksDirection=()=>banksIsMapReplay()?'Map-balanced minus collected':'Uniform minus collected';
const banksLabel=condition=>banksIsMapReplay() && condition==='collected_unique'?'Collected replay · archived baseline':banksIsMapReplay() && condition==='map_balanced'?'Map-balanced replay · new fit':coverageLabel(condition);
...
function renderBanksStudyCopy(){
  ...
  $('banks-exposure-panel').hidden=!map;
}
function renderBanksExposure(){
  if(!banksIsMapReplay())return;
  ...
  $('banks-exposure-caption').textContent='Actual minibatch state presentations, including repeats. The plotted share is each map’s presentations divided by that fit’s total. Support membership is unchanged. The treatment selects 64 distinct maps per update, changing both map exposure and within-batch diversity. Baseline counts are archived; treatment counts are new.';
...
function renderBanks(){
  stopBanksPlayback();const study=banksData(),run=study?.run || {},ready=Boolean(study?.aggregate?.length),mapReplay=banksIsMapReplay();
  renderBanksStudyCopy();
  $('banks-empty').hidden=ready;$('banks-content').hidden=!ready;
  if(!ready){$('banks-empty-title').textContent=run.status?.startsWith('inconsistent')?'Consistency check failed.':study?'No complete bank result available.':`No saved ${mapReplay?'map-balanced replay':'bank-replication'} result yet.`;$('banks-empty-message').textContent=study?`Status: ${(run.status || 'unavailable').replaceAll('_',' ')}. ${run.stop_reason || study.provenance?.stop_reason || 'No completed result is substituted.'}`:'Earlier studies remain available through the study selector.';for(const id of ['banks-effects','banks-success-chart','banks-efficient-chart','banks-steps-chart','banks-difference-chart'])$(id).innerHTML='';return;}
  const eligible=banksEligible();
  $('banks-note').textContent=run.status?.startsWith('inconsistent')?`Consistency check failed. No replication conclusion is available. ${run.stop_reason || study.provenance?.stop_reason || ''}`:run.status!=='complete'?'Incomplete bank comparison. Inspect completed measurements; missing bank pairs cannot be replaced by the pooled result.':!eligible?`Smoke or protocol-deviation run. Measurements verify execution and do not establish ${mapReplay?'a replay benefit':'bank replication'}.`:`${mapReplay?'The same collected supports are reused. Archived collected baselines are compared with new map-balanced fits.':'Inspect each independently constructed bank pair and its paired learner seeds.'} The equal-bank mean summarizes these pairs; it does not hide individual directions, create new competence gates, or establish significance or equivalence.`;
  $('banks-stats').innerHTML=[[number(banksRecords().length),mapReplay?'Reused collected banks':'New bank pairs',mapReplay?'Same states in baseline and treatment':'Each uniform bank matches its collected bank’s size'],[number(study.protocol?.seeds?.length),'Learner seeds per condition','Paired within each bank'],[number(banksPanels().length),'Shared fresh panels','Final policies evaluated on the same maps'],[number(banksRecords().length*(mapReplay?1:2)*(study.protocol?.seeds?.length || 0)),mapReplay?'New treatment fits':'Declared model fits',mapReplay?'Baseline fits are reused from the archive':'Same Double DQN setup; final-only evaluation']].map(([value,label,detail])=>`<div class="stat-card"><span>${label}</span><strong>${value}</strong><small>${detail}</small></div>`).join('');
  ...
  $('banks-method').textContent=`${number(run.wall_seconds)} seconds elapsed. ${mapReplay?'No new states are collected. Only map-balanced treatments are fitted; baseline models and training exposure come from the bank-replication archive. Both final policies are evaluated on this study’s new panels.':'New collected banks and matching uniform subsets are fitted with the same learner setup.'} Only final policies are evaluated. Intermediate model snapshots and training losses are archived for reproduction; they are not intermediate evaluation results.`;
...
function renderBanksComparison(){
  ...
  renderBanksComposition();renderBanksExposure();selectBanksTrajectory();
}
...
function renderBanksComposition(){
  ...
  $('banks-composition-caption').textContent=`Bank ${bank.bank_id}: ${number(bank.support_size)} included states per condition. Collection cost: ${number(collection.collection_episodes)} episodes and ${number(collection.collection_steps)} actual action steps. ${banksIsMapReplay()?'This is archived collection cost; no new collection is performed. Baseline and treatment have identical support.':'Uniform states are selected from the exhaustive universe; they are not additional collected visits.'} Support intersection: ${number(intersection.states)}; union ${number(intersection.union_states)}; Jaccard ${percent(intersection.jaccard)}.`;
...
async function loadData() {
  ...
  const results=await Promise.allSettled([getData(adaptationPath),getData('data/provenance.json'),getData('data/competence.json'),getData('data/supervised.json'),getData('data/fixed_targets.json'),getData('data/coverage.json'),getData('data/equal_support.json'),getData('data/panel_evaluation.json'),getData('data/bank_replication.json'),getData('data/map_replay.json')]);
  const errors=[];
  diagnostics=null;
  ...
  if(errors.length) $('load-status').textContent=errors.join(' · ');
    ...
    const loaded=[coverageStudies.map_replay&&'Map-balanced replay',coverageStudies.bank_replication&&'Bank replications',coverageStudies.panel_evaluation&&'Panel robustness',coverageStudies.equal_support&&'Equal-size banks',coverageStudies.coverage&&'Experience coverage',fixed&&'Fixed-data targets',supervised&&'Exact targets',competence&&'Competence',adaptation&&'Adaptation',provenance&&'Provenance'].filter(Boolean);
