...
const coverageStudies={coverage:null,equal_support:null,panel_evaluation:null,bank_replication:null,map_replay:null,within_map:null,recorded_actions:null};
...
const coverageColors={exhaustive:'#a6e5c1',collected_unique:'#98b8d1',uniform_subset:'#e7b985',map_balanced:'#a6e5c1',within_map_uniform:'#c4aadf',recorded_actions:'#d6ba83'};
const coverageLabel=condition=>({exhaustive:'Exhaustive states',collected_unique:'Collected unique states',uniform_subset:'Uniform subset',map_balanced:'Map-balanced replay',within_map_uniform:'Within-map uniform states',recorded_actions:'Recorded actions',shared:'Shared references'}[condition] || condition);
...
const banksHasArchivedControl=()=>banksIsMapReplay() || banksIsWithinMap() || banksIsRecorded();
const banksStudyKey=()=>banksIsRecorded()?'recorded_actions':banksIsWithinMap()?'within_map':banksIsMapReplay()?'map_replay':'bank_replication';
function banksData(){return coverageStudies[banksStudyKey()];}
const banksConditions=()=>['collected_unique',banksIsRecorded()?'recorded_actions':banksIsWithinMap()?'within_map_uniform':banksIsMapReplay()?'map_balanced':'uniform_subset'];
const banksDirection=()=>banksIsRecorded()?'Recorded minus all-action collected':banksIsWithinMap()?'Within-map uniform minus collected':banksIsMapReplay()?'Map-balanced minus collected':'Uniform minus collected';
const banksTreatmentShort=()=>banksIsRecorded()?'Recorded':banksIsWithinMap()?'Within-map':banksIsMapReplay()?'Balanced':'Uniform';
const banksLabel=condition=>banksHasArchivedControl() && condition==='collected_unique'?(banksIsRecorded()?'All-action collected · archived baseline':'Collected replay · archived baseline'):banksHasArchivedControl() && condition===banksConditions()[1]?`${coverageLabel(condition)} · new fit`:coverageLabel(condition);
...
function renderBanks(){
  ...
  $('banks-summary-table').innerHTML=`<thead><tr><th>Bank scope</th><th>Collected success</th><th>${banksTreatmentShort()} success</th><th>Collected efficient</th><th>${banksTreatmentShort()} efficient</th><th>Collected steps</th><th>${banksTreatmentShort()} steps</th><th>Efficient Δ</th></tr></thead><tbody>`+banksRecords().map(bank=>summaryRow(`Bank ${bank.bank_id}`,final(bank.bank_id,'collected_unique'),final(bank.bank_id,banksConditions()[1]),bankEffect(bank)?.mean_seed_efficient_success_rate_delta)).join('')+summaryRow('Equal-bank mean',study.pooled?.aggregate?.find(r=>r.condition==='collected_unique' && r.panel==='all' && r.mode==='greedy'),study.pooled?.aggregate?.find(r=>r.condition===banksConditions()[1] && r.panel==='all' && r.mode==='greedy'),mean?.mean_bank_efficient_success_rate_delta)+'</tbody>';
  const values=effects.slice(0,-1).map(r=>r.value).filter(Number.isFinite);
  $('banks-effects-caption').textContent=eligible?`Bank-mean efficiency differences: ${number(values.filter(v=>v>1e-12).length)} positive, ${number(values.filter(v=>v < -1e-12).length)} negative, ${number(values.filter(v=>Math.abs(v)<=1e-12).length)} tied (zero tolerance 10⁻¹²). These descriptive bank counts are not a significance test. All values above use final greedy evaluations; the mode toggle below does not change this primary comparison.`:`This run is not eligible for a ${recorded?'recorded-action':within?'within-map state-selection':mapReplay?'replay-benefit':'bank-replication'} interpretation. Saved primary measurements remain visible; no incomplete or smoke result is promoted by pooling.`;
  ...
  $('banks-method').textContent=`${number(run.wall_seconds)} seconds elapsed. ${mapReplay?`No new states are collected. Only ${recorded?'recorded-action':within?'within-map uniform':'map-balanced'} treatments are fitted; baseline models and training exposure come from the bank-replication archive. Both final policies are evaluated on this study’s new panels.`:'New collected banks and matching uniform subsets are fitted with the same learner setup.'} Only final policies are evaluated. Intermediate model snapshots and training losses are archived for reproduction; they are not intermediate evaluation results.`;
  ...
  $('banks-evidence-links').innerHTML=`<a href="data/${banksStudyKey()}.json" download>Download displayed data ↓</a>`+[['protocol_document','Study design'],['protocol','Saved protocol'],['report','Measured findings'],['evaluations','Raw final episodes'],['training',mapReplay?'New treatment loss log':'Training loss log'],['losses',mapReplay?'New treatment loss log':'Training loss log'],['exposure','Actual training exposure'],['banks','Bank composition'],['sampling','Sampling provenance'],['paired_differences','Paired differences'],['manifest','Artifact checksums']].flatMap(([key,label])=>{const path=study.artifacts?.[key];return typeof path==='string' && /^(docs|experiments)\/[a-zA-Z0-9_./-]+$/.test(path) && !path.split('/').includes('..')?[`<a href="../${escape(path)}">${label} ↗</a>`]:[];}).join('');
...
function renderBanksComparison(){
  ...
  renderBanksComposition();renderBanksExposure();renderBanksActions();selectBanksTrajectory();
}
...
function selectBanksTrajectory(){
  ...
  selectOptions('banks-replay-controller',[...new Set(atPanel.map(controller))].map(c=>[c,banksConditions().includes(c)?banksLabel(c):policyLabel(c)]),'collected_unique');const available=atPanel.filter(r=>controller(r)===$('banks-replay-controller').value);
...
async function loadData() {
  ...
  const results=await Promise.allSettled([getData(adaptationPath),getData('data/provenance.json'),getData('data/competence.json'),getData('data/supervised.json'),getData('data/fixed_targets.json'),getData('data/coverage.json'),getData('data/equal_support.json'),getData('data/panel_evaluation.json'),getData('data/bank_replication.json'),getData('data/map_replay.json'),getData('data/within_map.json'),getData('data/recorded_actions.json')]);
  const errors=[];
  diagnostics=null;
  ...
  if(errors.length) $('load-status').textContent=errors.join(' · ');
    ...
    const loaded=[coverageStudies.recorded_actions&&'Recorded actions',coverageStudies.within_map&&'Within-map states',coverageStudies.map_replay&&'Map-balanced replay',coverageStudies.bank_replication&&'Bank replications',coverageStudies.panel_evaluation&&'Panel robustness',coverageStudies.equal_support&&'Equal-size banks',coverageStudies.coverage&&'Experience coverage',fixed&&'Fixed-data targets',supervised&&'Exact targets',competence&&'Competence',adaptation&&'Adaptation',provenance&&'Provenance'].filter(Boolean);
